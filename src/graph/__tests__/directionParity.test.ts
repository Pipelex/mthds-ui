...
describe("direction parity: LR vs TB produce equivalent graph structures", () => {
  it.each(ALL_CATALOG_ENTRIES)("%s: same node IDs in both directions", async (_key, { spec }) => {
    const lr = await runFullPipeline(spec, { direction: "LR", showControllers: true });
    const tb = await runFullPipeline(spec, { direction: "TB", showControllers: true });
  ...
  it.each(ALL_CATALOG_ENTRIES)("%s: same edge IDs in both directions", async (_key, { spec }) => {
    const lr = await runFullPipeline(spec, { direction: "LR", showControllers: true });
    const tb = await runFullPipeline(spec, { direction: "TB", showControllers: true });
  ...
  it.each(ALL_CATALOG_ENTRIES)("%s: same node types in both directions", async (_key, { spec }) => {
    const lr = await runFullPipeline(spec, { direction: "LR", showControllers: true });
    const tb = await runFullPipeline(spec, { direction: "TB", showControllers: true });
