/**
 * Auto-generated by scripts/generate-fixtures.mjs from pipelex DRY runs.
 * DO NOT EDIT — regenerate with `make fixtures`.
 */
import type { GraphSpec } from "@graph/types";

export const DRY_CV_SCREENING = {
  graph_id: "25618ec2-6c0e-4596-aad2-5f2561785b30",
  created_at: "2026-08-14T09:49:48.083281Z",
  pipeline_ref: { domain: "recruitment", main_pipe: "cv_screening", entrypoint: null },
  nodes: [
    {
      id: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_0",
      kind: "controller",
      pipe_code: "cv_screening",
      pipe_type: "PipeSequence",
      description: "Full CV screening pipeline",
      domain_code: "recruitment",
      status: "succeeded",
      skip_reason: null,
      timing: {
        started_at: "2026-08-14T09:49:48.083281Z",
        ended_at: "2026-08-14T09:49:48.677603Z",
        duration: 0.594322,
      },
      io: {
        inputs: [
          {
            name: "cv",
            concept: "Document",
            content_type: "NMvjSqhruIcIeiVwhQdC",
            preview: null,
            size: null,
            digest: "2GVPS",
            data: {
              url: "OOntSZCvIhnmXqdRiBUB",
              public_url: "ttPWesjNaHyoltldEDdU",
              mime_type: "NMvjSqhruIcIeiVwhQdC",
              filename: "gCsLXbUrsdbvEcEdmENA",
              title: "VMCyaCjOISQSWDSEumDB",
              snippet: "dMgHnVxqpiiLfIGMOypy",
            },
            data_text: "VMCyaCjOISQSWDSEumDB (OOntSZCvIhnmXqdRiBUB)\n  dMgHnVxqpiiLfIGMOypy\n",
            data_html:
              '<a href="ttPWesjNaHyoltldEDdU" class="msg-document">VMCyaCjOISQSWDSEumDB</a><br/><small>dMgHnVxqpiiLfIGMOypy</small>',
            extra: {},
          },
        ],
        outputs: [
          {
            name: "report",
            concept: "Report",
            content_type: null,
            preview: null,
            size: null,
            digest: "fgP57",
            data: {
              text: "# Candidate Screening Report\n\n## Candidate: XQnJeirSOmfSIDuFlwPb\nVLSsAMGooTVMzyADqWSJ\n\n## Match Score: -21601.2201684928/100\nQyNVlaFAWBNAVTyTxQeJ\n\n## Strengths\nmatch.strengths: ```\n['b63858c47c']\n```\n\n## Gaps\nmatch.gaps: ```\n['4c07b291e3']\n```",
            },
            data_text:
              "                                     Candidate Screening Report                                     \n\nCandidate: XQnJeirSOmfSIDuFlwPb                                                                     \n\nVLSsAMGooTVMzyADqWSJ                                                                                \n\nMatch Score: -21601.2201684928/100                                                                  \n\nQyNVlaFAWBNAVTyTxQeJ                                                                                \n\nStrengths                                                                                           \n\nmatch.strengths: ``` ['b63858c47c']                                                                 \n\n                                                                                                    \n                                                                                                    \n ## Gaps                                                                                            \n match.gaps: ```                                                                                    \n ['4c07b291e3']                                                                                     \n                                                                                                    \n",
            data_html:
              "# Candidate Screening Report\n\n## Candidate: XQnJeirSOmfSIDuFlwPb\nVLSsAMGooTVMzyADqWSJ\n\n## Match Score: -21601.2201684928/100\nQyNVlaFAWBNAVTyTxQeJ\n\n## Strengths\nmatch.strengths: ```\n[&#x27;b63858c47c&#x27;]\n```\n\n## Gaps\nmatch.gaps: ```\n[&#x27;4c07b291e3&#x27;]\n```",
            extra: {},
          },
        ],
      },
      error: null,
      tags: {},
      metrics: {},
      usage: {
        inference_calls: 0,
        rated_inference_calls: 0,
        nb_tokens_by_category: {},
        total_tokens: 0,
        cost: null,
        cost_input: null,
        cost_output: null,
        by_model: [],
        subtree_inference_calls: 2,
        subtree_rated_inference_calls: 0,
        subtree_nb_tokens_by_category: {},
        subtree_total_tokens: 0,
        subtree_cost: null,
        subtree_cost_input: null,
        subtree_cost_output: null,
        subtree_by_model: [
          {
            inference_model_name: "dry_run",
            inference_model_id: "dry_run",
            model_type: "llm",
            inference_calls: 2,
            rated_inference_calls: 0,
            cost: null,
          },
        ],
      },
      execution_data: { step_count: 5 },
    },
    {
      id: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_1",
      kind: "operator",
      pipe_code: "extract_cv",
      pipe_type: "PipeExtract",
      description: "Extract pages from CV document",
      domain_code: "recruitment",
      status: "succeeded",
      skip_reason: null,
      timing: {
        started_at: "2026-08-14T09:49:48.094804Z",
        ended_at: "2026-08-14T09:49:48.110904Z",
        duration: 0.0161,
      },
      io: {
        inputs: [
          {
            name: "cv",
            concept: "Document",
            content_type: "NMvjSqhruIcIeiVwhQdC",
            preview: null,
            size: null,
            digest: "2GVPS",
            data: {
              url: "OOntSZCvIhnmXqdRiBUB",
              public_url: "ttPWesjNaHyoltldEDdU",
              mime_type: "NMvjSqhruIcIeiVwhQdC",
              filename: "gCsLXbUrsdbvEcEdmENA",
              title: "VMCyaCjOISQSWDSEumDB",
              snippet: "dMgHnVxqpiiLfIGMOypy",
            },
            data_text: "VMCyaCjOISQSWDSEumDB (OOntSZCvIhnmXqdRiBUB)\n  dMgHnVxqpiiLfIGMOypy\n",
            data_html:
              '<a href="ttPWesjNaHyoltldEDdU" class="msg-document">VMCyaCjOISQSWDSEumDB</a><br/><small>dMgHnVxqpiiLfIGMOypy</small>',
            extra: {},
          },
        ],
        outputs: [
          {
            name: "pages",
            concept: "Page",
            content_type: null,
            preview: null,
            size: null,
            digest: "4mTEr",
            data: {
              items: [
                {
                  text_and_images: {
                    text: { text: "DRY RUN: OCR text" },
                    images: [],
                    raw_html: null,
                  },
                  page_view: null,
                },
                {
                  text_and_images: {
                    text: { text: "DRY RUN: OCR text" },
                    images: [],
                    raw_html: null,
                  },
                  page_view: null,
                },
                {
                  text_and_images: {
                    text: { text: "DRY RUN: OCR text" },
                    images: [],
                    raw_html: null,
                  },
                  page_view: null,
                },
                {
                  text_and_images: {
                    text: { text: "DRY RUN: OCR text" },
                    images: [],
                    raw_html: null,
                  },
                  page_view: null,
                },
              ],
            },
            data_text:
              "   1    │ DRY RUN: OCR text                                                     \n────────┼───────────────────────────────────────────────────────────────────────\n   2    │ DRY RUN: OCR text                                                     \n────────┼───────────────────────────────────────────────────────────────────────\n   3    │ DRY RUN: OCR text                                                     \n────────┼───────────────────────────────────────────────────────────────────────\n   4    │ DRY RUN: OCR text                                                     \n",
            data_html:
              "<ul><li><table><tr><th>text_and_images</th><td>DRY RUN: OCR text</td></tr></table></li><li><table><tr><th>text_and_images</th><td>DRY RUN: OCR text</td></tr></table></li><li><table><tr><th>text_and_images</th><td>DRY RUN: OCR text</td></tr></table></li><li><table><tr><th>text_and_images</th><td>DRY RUN: OCR text</td></tr></table></li></ul>",
            extra: {},
          },
        ],
      },
      error: null,
      tags: {},
      metrics: {},
      usage: {
        inference_calls: 0,
        rated_inference_calls: 0,
        nb_tokens_by_category: {},
        total_tokens: 0,
        cost: null,
        cost_input: null,
        cost_output: null,
        by_model: [],
        subtree_inference_calls: 0,
        subtree_rated_inference_calls: 0,
        subtree_nb_tokens_by_category: {},
        subtree_total_tokens: 0,
        subtree_cost: null,
        subtree_cost_input: null,
        subtree_cost_output: null,
        subtree_by_model: [],
      },
      execution_data: {
        resolved_model: "azure-document-intelligence",
        document_stuff_name: "cv",
        should_caption_images: false,
        should_include_page_views: false,
      },
    },
    {
      id: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_2",
      kind: "operator",
      pipe_code: "analyze_candidate",
      pipe_type: "PipeLLM",
      description: "Analyze CV to build candidate profile",
      domain_code: "recruitment",
      status: "succeeded",
      skip_reason: null,
      timing: {
        started_at: "2026-08-14T09:49:48.116487Z",
        ended_at: "2026-08-14T09:49:48.142312Z",
        duration: 0.025825,
      },
      io: {
        inputs: [
          {
            name: "pages",
            concept: "Page",
            content_type: null,
            preview: null,
            size: null,
            digest: "4mTEr",
            data: {
              items: [
                {
                  text_and_images: {
                    text: { text: "DRY RUN: OCR text" },
                    images: [],
                    raw_html: null,
                  },
                  page_view: null,
                },
                {
                  text_and_images: {
                    text: { text: "DRY RUN: OCR text" },
                    images: [],
                    raw_html: null,
                  },
                  page_view: null,
                },
                {
                  text_and_images: {
                    text: { text: "DRY RUN: OCR text" },
                    images: [],
                    raw_html: null,
                  },
                  page_view: null,
                },
                {
                  text_and_images: {
                    text: { text: "DRY RUN: OCR text" },
                    images: [],
                    raw_html: null,
                  },
                  page_view: null,
                },
              ],
            },
            data_text:
              "   1    │ DRY RUN: OCR text                                                     \n────────┼───────────────────────────────────────────────────────────────────────\n   2    │ DRY RUN: OCR text                                                     \n────────┼───────────────────────────────────────────────────────────────────────\n   3    │ DRY RUN: OCR text                                                     \n────────┼───────────────────────────────────────────────────────────────────────\n   4    │ DRY RUN: OCR text                                                     \n",
            data_html:
              "<ul><li><table><tr><th>text_and_images</th><td>DRY RUN: OCR text</td></tr></table></li><li><table><tr><th>text_and_images</th><td>DRY RUN: OCR text</td></tr></table></li><li><table><tr><th>text_and_images</th><td>DRY RUN: OCR text</td></tr></table></li><li><table><tr><th>text_and_images</th><td>DRY RUN: OCR text</td></tr></table></li></ul>",
            extra: {},
          },
        ],
        outputs: [
          {
            name: "profile",
            concept: "CandidateProfile",
            content_type: null,
            preview: null,
            size: null,
            digest: "cW76c",
            data: {
              name: "XQnJeirSOmfSIDuFlwPb",
              skills: ["c0f1"],
              experience_years: 4453,
              summary: "VLSsAMGooTVMzyADqWSJ",
            },
            data_text:
              " Attribute                          ┃ Value                                     \n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n name                               │ XQnJeirSOmfSIDuFlwPb                      \n────────────────────────────────────┼───────────────────────────────────────────\n skills                             │   1   │ c0f1                              \n────────────────────────────────────┼───────────────────────────────────────────\n experience_years                   │ 4453                                      \n────────────────────────────────────┼───────────────────────────────────────────\n summary                            │ VLSsAMGooTVMzyADqWSJ                      \n",
            data_html:
              "<table><tr><th>name</th><td>XQnJeirSOmfSIDuFlwPb</td></tr><tr><th>skills</th><td><ul><li>c0f1</li></ul></td></tr><tr><th>experience_years</th><td>4453</td></tr><tr><th>summary</th><td>VLSsAMGooTVMzyADqWSJ</td></tr></table>",
            extra: {},
          },
        ],
      },
      error: null,
      tags: {},
      metrics: {},
      usage: {
        inference_calls: 1,
        rated_inference_calls: 0,
        nb_tokens_by_category: {},
        total_tokens: 0,
        cost: null,
        cost_input: null,
        cost_output: null,
        by_model: [
          {
            inference_model_name: "dry_run",
            inference_model_id: "dry_run",
            model_type: "llm",
            inference_calls: 1,
            rated_inference_calls: 0,
            cost: null,
          },
        ],
        subtree_inference_calls: 1,
        subtree_rated_inference_calls: 0,
        subtree_nb_tokens_by_category: {},
        subtree_total_tokens: 0,
        subtree_cost: null,
        subtree_cost_input: null,
        subtree_cost_output: null,
        subtree_by_model: [
          {
            inference_model_name: "dry_run",
            inference_model_id: "dry_run",
            model_type: "llm",
            inference_calls: 1,
            rated_inference_calls: 0,
            cost: null,
          },
        ],
      },
      execution_data: {
        resolved_model: "claude-4.6-sonnet",
        resolved_model_for_object: "claude-4.6-sonnet",
        is_multiple_output: false,
        rendered_system_prompt: null,
        rendered_user_prompt:
          'Analyze this CV and extract a structured candidate profile:\n\n<pages>\n\n • item #1:\n\n# text_and_images\n\n## text: ### text: DRY RUN: OCR text\n\n## images: \n\n## raw_html: None\n\n# page_view: None\n\n • item #2:\n\n# text_and_images\n\n## text: ### text: DRY RUN: OCR text\n\n## images: \n\n## raw_html: None\n\n# page_view: None\n\n • item #3:\n\n# text_and_images\n\n## text: ### text: DRY RUN: OCR text\n\n## images: \n\n## raw_html: None\n\n# page_view: None\n\n • item #4:\n\n# text_and_images\n\n## text: ### text: DRY RUN: OCR text\n\n## images: \n\n## raw_html: None\n\n# page_view: None\n\n</pages>\n\n---\nThe instance we want to generate will be for the following class:\nclass recruitment__CandidateProfile(StructuredContent):\n    """Structured profile of a job candidate"""\n    name: str  # Full name\n    skills: List[Any] | None = None  # Key skills\n    experience_years: int | None = None  # Years of experience\n    summary: str  # Brief profile summary\n\nDO NOT create information.\nIf some information is not present for an attribute, output the default value or None according to the attribute definition.',
        structuring_path: "object_direct",
      },
    },
    {
      id: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_3",
      kind: "controller",
      pipe_code: "enrich_candidate",
      pipe_type: "PipeParallel",
      description: "Search online and generate profile card in parallel",
      domain_code: "recruitment",
      status: "succeeded",
      skip_reason: null,
      timing: {
        started_at: "2026-08-14T09:49:48.148530Z",
        ended_at: "2026-08-14T09:49:48.594943Z",
        duration: 0.446413,
      },
      io: {
        inputs: [
          {
            name: "profile",
            concept: "CandidateProfile",
            content_type: null,
            preview: null,
            size: null,
            digest: "cW76c",
            data: {
              name: "XQnJeirSOmfSIDuFlwPb",
              skills: ["c0f1"],
              experience_years: 4453,
              summary: "VLSsAMGooTVMzyADqWSJ",
            },
            data_text:
              " Attribute                          ┃ Value                                     \n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n name                               │ XQnJeirSOmfSIDuFlwPb                      \n────────────────────────────────────┼───────────────────────────────────────────\n skills                             │   1   │ c0f1                              \n────────────────────────────────────┼───────────────────────────────────────────\n experience_years                   │ 4453                                      \n────────────────────────────────────┼───────────────────────────────────────────\n summary                            │ VLSsAMGooTVMzyADqWSJ                      \n",
            data_html:
              "<table><tr><th>name</th><td>XQnJeirSOmfSIDuFlwPb</td></tr><tr><th>skills</th><td><ul><li>c0f1</li></ul></td></tr><tr><th>experience_years</th><td>4453</td></tr><tr><th>summary</th><td>VLSsAMGooTVMzyADqWSJ</td></tr></table>",
            extra: {},
          },
        ],
        outputs: [
          {
            name: "search_result",
            concept: "SearchResult",
            content_type: null,
            preview: null,
            size: null,
            digest: "UVyq5",
            data: {
              answer: "LDdKASaojiqARIDCJSTw",
              sources: [
                {
                  url: "hgyxAfcijBKdPtAmfcha",
                  public_url: "xKNISWXPVVGLTtyBWElJ",
                  mime_type: "ooofYrGnFvYWxgyoqlTJ",
                  filename: "SAPGrYOVYLJFETyEVMqB",
                  title: "UhCicBqerKciCYyjIaGd",
                  snippet: "rOiCMWsvWqZvgDZVIlxq",
                },
                {
                  url: "WhpIemcSTuEFrLEYWZvt",
                  public_url: "kFFrTiFLjDBaKEEzcXXk",
                  mime_type: "fgVXKJzyhNfOltQoyrcr",
                  filename: "LrfoWSxJKHhKvKlyPsXW",
                  title: "AeoPypjpJywBcTxVIhyu",
                  snippet: "ZDAAiGVkZkKoMvxgWVQd",
                },
                {
                  url: "TDdFFqXcvTFIfXiPLUYH",
                  public_url: "vLBOoQgeFvfQROcLULjG",
                  mime_type: "FTBIwcSRHIhcaqTWqQat",
                  filename: "rXTsfCfjKdcNjZPNFApN",
                  title: "BgkfWftmSmtFfKNxNYtZ",
                  snippet: "GrxmpaYeyqasSdUkpxaW",
                },
              ],
            },
            data_text:
              "Search Result:\nLDdKASaojiqARIDCJSTw                                                                                \n\nSources (3):\nUhCicBqerKciCYyjIaGd (hgyxAfcijBKdPtAmfcha)\n  rOiCMWsvWqZvgDZVIlxq\n\nAeoPypjpJywBcTxVIhyu (WhpIemcSTuEFrLEYWZvt)\n  ZDAAiGVkZkKoMvxgWVQd\n\nBgkfWftmSmtFfKNxNYtZ (TDdFFqXcvTFIfXiPLUYH)\n  GrxmpaYeyqasSdUkpxaW\n",
            data_html:
              '<div><p>LDdKASaojiqARIDCJSTw</p><h4>Sources</h4><ul><li><a href="xKNISWXPVVGLTtyBWElJ" class="msg-document">UhCicBqerKciCYyjIaGd</a><br/><small>rOiCMWsvWqZvgDZVIlxq</small></li><li><a href="kFFrTiFLjDBaKEEzcXXk" class="msg-document">AeoPypjpJywBcTxVIhyu</a><br/><small>ZDAAiGVkZkKoMvxgWVQd</small></li><li><a href="vLBOoQgeFvfQROcLULjG" class="msg-document">BgkfWftmSmtFfKNxNYtZ</a><br/><small>GrxmpaYeyqasSdUkpxaW</small></li></ul></div>',
            extra: {},
          },
          {
            name: "card_image",
            concept: "Image",
            content_type: "image/jpeg",
            preview: null,
            size: null,
            digest: "drTUo",
            data: {
              url: "https://storage.googleapis.com/public_test_files_7fa6_4277_9ab/fashion/fashion_photo_1.jpg",
              public_url:
                "https://storage.googleapis.com/public_test_files_7fa6_4277_9ab/fashion/fashion_photo_1.jpg",
              source_prompt:
                "Professional profile card for XQnJeirSOmfSIDuFlwPb, VLSsAMGooTVMzyADqWSJ",
              source_negative_prompt: null,
              caption: null,
              mime_type: "image/jpeg",
              width: 1024,
              height: 1024,
              filename: null,
            },
            data_text:
              "Image:\nURL: https://storage.googleapis.com/public_test_files_7fa6_4277_9ab/fashion/fashion_photo_1.jpg     \nPublic URL: Open Image\nSize: 1024x1024\nMIME Type: image/jpeg\n\nSource Prompt:\nProfessional profile card for XQnJeirSOmfSIDuFlwPb, VLSsAMGooTVMzyADqWSJ\n",
            data_html:
              '<img src="https://storage.googleapis.com/public_test_files_7fa6_4277_9ab/fashion/fashion_photo_1.jpg" class="msg-img">',
            extra: {},
          },
          {
            name: "candidate_assets",
            concept: "Composite",
            content_type: null,
            preview: null,
            size: null,
            digest: "98AF9",
            data: {
              search_result: {
                answer: "LDdKASaojiqARIDCJSTw",
                sources: [
                  {
                    url: "hgyxAfcijBKdPtAmfcha",
                    public_url: "xKNISWXPVVGLTtyBWElJ",
                    mime_type: "ooofYrGnFvYWxgyoqlTJ",
                    filename: "SAPGrYOVYLJFETyEVMqB",
                    title: "UhCicBqerKciCYyjIaGd",
                    snippet: "rOiCMWsvWqZvgDZVIlxq",
                  },
                  {
                    url: "WhpIemcSTuEFrLEYWZvt",
                    public_url: "kFFrTiFLjDBaKEEzcXXk",
                    mime_type: "fgVXKJzyhNfOltQoyrcr",
                    filename: "LrfoWSxJKHhKvKlyPsXW",
                    title: "AeoPypjpJywBcTxVIhyu",
                    snippet: "ZDAAiGVkZkKoMvxgWVQd",
                  },
                  {
                    url: "TDdFFqXcvTFIfXiPLUYH",
                    public_url: "vLBOoQgeFvfQROcLULjG",
                    mime_type: "FTBIwcSRHIhcaqTWqQat",
                    filename: "rXTsfCfjKdcNjZPNFApN",
                    title: "BgkfWftmSmtFfKNxNYtZ",
                    snippet: "GrxmpaYeyqasSdUkpxaW",
                  },
                ],
              },
              card_image: {
                url: "https://storage.googleapis.com/public_test_files_7fa6_4277_9ab/fashion/fashion_photo_1.jpg",
                public_url:
                  "https://storage.googleapis.com/public_test_files_7fa6_4277_9ab/fashion/fashion_photo_1.jpg",
                source_prompt:
                  "Professional profile card for XQnJeirSOmfSIDuFlwPb, VLSsAMGooTVMzyADqWSJ",
                source_negative_prompt: null,
                caption: null,
                mime_type: "image/jpeg",
                width: 1024,
                height: 1024,
                filename: null,
              },
            },
            data_text:
              '{\n    "search_result": {\n        "answer": "LDdKASaojiqARIDCJSTw",\n        "sources": [\n            {\n                "url": "hgyxAfcijBKdPtAmfcha",\n                "public_url": "xKNISWXPVVGLTtyBWElJ",\n                "mime_type": "ooofYrGnFvYWxgyoqlTJ",\n                "filename": "SAPGrYOVYLJFETyEVMqB",\n                "title": "UhCicBqerKciCYyjIaGd",\n                "snippet": "rOiCMWsvWqZvgDZVIlxq"\n            },\n            {\n                "url": "WhpIemcSTuEFrLEYWZvt",\n                "public_url": "kFFrTiFLjDBaKEEzcXXk",\n                "mime_type": "fgVXKJzyhNfOltQoyrcr",\n                "filename": "LrfoWSxJKHhKvKlyPsXW",\n                "title": "AeoPypjpJywBcTxVIhyu",\n                "snippet": "ZDAAiGVkZkKoMvxgWVQd"\n            },\n            {\n                "url": "TDdFFqXcvTFIfXiPLUYH",\n                "public_url": "vLBOoQgeFvfQROcLULjG",\n                "mime_type": "FTBIwcSRHIhcaqTWqQat",\n                "filename": "rXTsfCfjKdcNjZPNFApN",\n                "title": "BgkfWftmSmtFfKNxNYtZ",\n                "snippet": "GrxmpaYeyqasSdUkpxaW"\n            }\n        ]\n    },\n    "card_image": {\n        "url": \n"https://storage.googleapis.com/public_test_files_7fa6_4277_9ab/fashion/fashion_photo_1.jpg",\n        "public_url": \n"https://storage.googleapis.com/public_test_files_7fa6_4277_9ab/fashion/fashion_photo_1.jpg",\n        "source_prompt": "Professional profile card for XQnJeirSOmfSIDuFlwPb, VLSsAMGooTVMzyADqWSJ",\n        "source_negative_prompt": null,\n        "caption": null,\n        "mime_type": "image/jpeg",\n        "width": 1024,\n        "height": 1024,\n        "filename": null\n    }\n}\n',
            data_html:
              '<table><tr><th>search_result</th><td><div><p>LDdKASaojiqARIDCJSTw</p><h4>Sources</h4><ul><li><a href="xKNISWXPVVGLTtyBWElJ" class="msg-document">UhCicBqerKciCYyjIaGd</a><br/><small>rOiCMWsvWqZvgDZVIlxq</small></li><li><a href="kFFrTiFLjDBaKEEzcXXk" class="msg-document">AeoPypjpJywBcTxVIhyu</a><br/><small>ZDAAiGVkZkKoMvxgWVQd</small></li><li><a href="vLBOoQgeFvfQROcLULjG" class="msg-document">BgkfWftmSmtFfKNxNYtZ</a><br/><small>GrxmpaYeyqasSdUkpxaW</small></li></ul></div></td></tr><tr><th>card_image</th><td><img src="https://storage.googleapis.com/public_test_files_7fa6_4277_9ab/fashion/fashion_photo_1.jpg" class="msg-img"></td></tr></table>',
            extra: {},
          },
        ],
      },
      error: null,
      tags: {},
      metrics: {},
      usage: {
        inference_calls: 0,
        rated_inference_calls: 0,
        nb_tokens_by_category: {},
        total_tokens: 0,
        cost: null,
        cost_input: null,
        cost_output: null,
        by_model: [],
        subtree_inference_calls: 0,
        subtree_rated_inference_calls: 0,
        subtree_nb_tokens_by_category: {},
        subtree_total_tokens: 0,
        subtree_cost: null,
        subtree_cost_input: null,
        subtree_cost_output: null,
        subtree_by_model: [],
      },
      execution_data: {
        branch_count: 2,
        add_each_output: true,
        combined_output_concept: "native.Composite",
      },
    },
    {
      id: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_4",
      kind: "operator",
      pipe_code: "search_candidate",
      pipe_type: "PipeSearch",
      description: "Search for candidate online presence",
      domain_code: "recruitment",
      status: "succeeded",
      skip_reason: null,
      timing: {
        started_at: "2026-08-14T09:49:48.155653Z",
        ended_at: "2026-08-14T09:49:48.338020Z",
        duration: 0.182367,
      },
      io: {
        inputs: [
          {
            name: "profile",
            concept: "CandidateProfile",
            content_type: null,
            preview: null,
            size: null,
            digest: "cW76c",
            data: {
              name: "XQnJeirSOmfSIDuFlwPb",
              skills: ["c0f1"],
              experience_years: 4453,
              summary: "VLSsAMGooTVMzyADqWSJ",
            },
            data_text:
              " Attribute                          ┃ Value                                     \n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n name                               │ XQnJeirSOmfSIDuFlwPb                      \n────────────────────────────────────┼───────────────────────────────────────────\n skills                             │   1   │ c0f1                              \n────────────────────────────────────┼───────────────────────────────────────────\n experience_years                   │ 4453                                      \n────────────────────────────────────┼───────────────────────────────────────────\n summary                            │ VLSsAMGooTVMzyADqWSJ                      \n",
            data_html:
              "<table><tr><th>name</th><td>XQnJeirSOmfSIDuFlwPb</td></tr><tr><th>skills</th><td><ul><li>c0f1</li></ul></td></tr><tr><th>experience_years</th><td>4453</td></tr><tr><th>summary</th><td>VLSsAMGooTVMzyADqWSJ</td></tr></table>",
            extra: {},
          },
        ],
        outputs: [
          {
            name: "search_result",
            concept: "SearchResult",
            content_type: null,
            preview: null,
            size: null,
            digest: "UVyq5",
            data: {
              answer: "LDdKASaojiqARIDCJSTw",
              sources: [
                {
                  url: "hgyxAfcijBKdPtAmfcha",
                  public_url: "xKNISWXPVVGLTtyBWElJ",
                  mime_type: "ooofYrGnFvYWxgyoqlTJ",
                  filename: "SAPGrYOVYLJFETyEVMqB",
                  title: "UhCicBqerKciCYyjIaGd",
                  snippet: "rOiCMWsvWqZvgDZVIlxq",
                },
                {
                  url: "WhpIemcSTuEFrLEYWZvt",
                  public_url: "kFFrTiFLjDBaKEEzcXXk",
                  mime_type: "fgVXKJzyhNfOltQoyrcr",
                  filename: "LrfoWSxJKHhKvKlyPsXW",
                  title: "AeoPypjpJywBcTxVIhyu",
                  snippet: "ZDAAiGVkZkKoMvxgWVQd",
                },
                {
                  url: "TDdFFqXcvTFIfXiPLUYH",
                  public_url: "vLBOoQgeFvfQROcLULjG",
                  mime_type: "FTBIwcSRHIhcaqTWqQat",
                  filename: "rXTsfCfjKdcNjZPNFApN",
                  title: "BgkfWftmSmtFfKNxNYtZ",
                  snippet: "GrxmpaYeyqasSdUkpxaW",
                },
              ],
            },
            data_text:
              "Search Result:\nLDdKASaojiqARIDCJSTw                                                                                \n\nSources (3):\nUhCicBqerKciCYyjIaGd (hgyxAfcijBKdPtAmfcha)\n  rOiCMWsvWqZvgDZVIlxq\n\nAeoPypjpJywBcTxVIhyu (WhpIemcSTuEFrLEYWZvt)\n  ZDAAiGVkZkKoMvxgWVQd\n\nBgkfWftmSmtFfKNxNYtZ (TDdFFqXcvTFIfXiPLUYH)\n  GrxmpaYeyqasSdUkpxaW\n",
            data_html:
              '<div><p>LDdKASaojiqARIDCJSTw</p><h4>Sources</h4><ul><li><a href="xKNISWXPVVGLTtyBWElJ" class="msg-document">UhCicBqerKciCYyjIaGd</a><br/><small>rOiCMWsvWqZvgDZVIlxq</small></li><li><a href="kFFrTiFLjDBaKEEzcXXk" class="msg-document">AeoPypjpJywBcTxVIhyu</a><br/><small>ZDAAiGVkZkKoMvxgWVQd</small></li><li><a href="vLBOoQgeFvfQROcLULjG" class="msg-document">BgkfWftmSmtFfKNxNYtZ</a><br/><small>GrxmpaYeyqasSdUkpxaW</small></li></ul></div>',
            extra: {},
          },
        ],
      },
      error: null,
      tags: {},
      metrics: {},
      usage: {
        inference_calls: 0,
        rated_inference_calls: 0,
        nb_tokens_by_category: {},
        total_tokens: 0,
        cost: null,
        cost_input: null,
        cost_output: null,
        by_model: [],
        subtree_inference_calls: 0,
        subtree_rated_inference_calls: 0,
        subtree_nb_tokens_by_category: {},
        subtree_total_tokens: 0,
        subtree_cost: null,
        subtree_cost_input: null,
        subtree_cost_output: null,
        subtree_by_model: [],
      },
      execution_data: {
        rendered_query: "XQnJeirSOmfSIDuFlwPb professional background",
        resolved_model: "linkup-standard",
        is_structured_output: false,
      },
    },
    {
      id: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_5",
      kind: "operator",
      pipe_code: "generate_card",
      pipe_type: "PipeImgGen",
      description: "Generate candidate profile card image",
      domain_code: "recruitment",
      status: "succeeded",
      skip_reason: null,
      timing: {
        started_at: "2026-08-14T09:49:48.396320Z",
        ended_at: "2026-08-14T09:49:48.479504Z",
        duration: 0.083184,
      },
      io: {
        inputs: [
          {
            name: "profile",
            concept: "CandidateProfile",
            content_type: null,
            preview: null,
            size: null,
            digest: "cW76c",
            data: {
              name: "XQnJeirSOmfSIDuFlwPb",
              skills: ["c0f1"],
              experience_years: 4453,
              summary: "VLSsAMGooTVMzyADqWSJ",
            },
            data_text:
              " Attribute                          ┃ Value                                     \n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n name                               │ XQnJeirSOmfSIDuFlwPb                      \n────────────────────────────────────┼───────────────────────────────────────────\n skills                             │   1   │ c0f1                              \n────────────────────────────────────┼───────────────────────────────────────────\n experience_years                   │ 4453                                      \n────────────────────────────────────┼───────────────────────────────────────────\n summary                            │ VLSsAMGooTVMzyADqWSJ                      \n",
            data_html:
              "<table><tr><th>name</th><td>XQnJeirSOmfSIDuFlwPb</td></tr><tr><th>skills</th><td><ul><li>c0f1</li></ul></td></tr><tr><th>experience_years</th><td>4453</td></tr><tr><th>summary</th><td>VLSsAMGooTVMzyADqWSJ</td></tr></table>",
            extra: {},
          },
        ],
        outputs: [
          {
            name: "card_image",
            concept: "Image",
            content_type: "image/jpeg",
            preview: null,
            size: null,
            digest: "drTUo",
            data: {
              url: "https://storage.googleapis.com/public_test_files_7fa6_4277_9ab/fashion/fashion_photo_1.jpg",
              public_url:
                "https://storage.googleapis.com/public_test_files_7fa6_4277_9ab/fashion/fashion_photo_1.jpg",
              source_prompt:
                "Professional profile card for XQnJeirSOmfSIDuFlwPb, VLSsAMGooTVMzyADqWSJ",
              source_negative_prompt: null,
              caption: null,
              mime_type: "image/jpeg",
              width: 1024,
              height: 1024,
              filename: null,
            },
            data_text:
              "Image:\nURL: https://storage.googleapis.com/public_test_files_7fa6_4277_9ab/fashion/fashion_photo_1.jpg     \nPublic URL: Open Image\nSize: 1024x1024\nMIME Type: image/jpeg\n\nSource Prompt:\nProfessional profile card for XQnJeirSOmfSIDuFlwPb, VLSsAMGooTVMzyADqWSJ\n",
            data_html:
              '<img src="https://storage.googleapis.com/public_test_files_7fa6_4277_9ab/fashion/fashion_photo_1.jpg" class="msg-img">',
            extra: {},
          },
        ],
      },
      error: null,
      tags: {},
      metrics: {},
      usage: {
        inference_calls: 0,
        rated_inference_calls: 0,
        nb_tokens_by_category: {},
        total_tokens: 0,
        cost: null,
        cost_input: null,
        cost_output: null,
        by_model: [],
        subtree_inference_calls: 0,
        subtree_rated_inference_calls: 0,
        subtree_nb_tokens_by_category: {},
        subtree_total_tokens: 0,
        subtree_cost: null,
        subtree_cost_input: null,
        subtree_cost_output: null,
        subtree_by_model: [],
      },
      execution_data: {
        resolved_model: "@default-general",
        rendered_prompt: "Professional profile card for XQnJeirSOmfSIDuFlwPb, VLSsAMGooTVMzyADqWSJ",
        rendered_negative_prompt: null,
        aspect_ratio: "square",
        nb_images: 1,
      },
    },
    {
      id: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_6",
      kind: "operator",
      pipe_code: "score_match",
      pipe_type: "PipeLLM",
      description: "Score candidate-role match",
      domain_code: "recruitment",
      status: "succeeded",
      skip_reason: null,
      timing: {
        started_at: "2026-08-14T09:49:48.608466Z",
        ended_at: "2026-08-14T09:49:48.641373Z",
        duration: 0.032907,
      },
      io: {
        inputs: [
          {
            name: "profile",
            concept: "CandidateProfile",
            content_type: null,
            preview: null,
            size: null,
            digest: "cW76c",
            data: {
              name: "XQnJeirSOmfSIDuFlwPb",
              skills: ["c0f1"],
              experience_years: 4453,
              summary: "VLSsAMGooTVMzyADqWSJ",
            },
            data_text:
              " Attribute                          ┃ Value                                     \n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n name                               │ XQnJeirSOmfSIDuFlwPb                      \n────────────────────────────────────┼───────────────────────────────────────────\n skills                             │   1   │ c0f1                              \n────────────────────────────────────┼───────────────────────────────────────────\n experience_years                   │ 4453                                      \n────────────────────────────────────┼───────────────────────────────────────────\n summary                            │ VLSsAMGooTVMzyADqWSJ                      \n",
            data_html:
              "<table><tr><th>name</th><td>XQnJeirSOmfSIDuFlwPb</td></tr><tr><th>skills</th><td><ul><li>c0f1</li></ul></td></tr><tr><th>experience_years</th><td>4453</td></tr><tr><th>summary</th><td>VLSsAMGooTVMzyADqWSJ</td></tr></table>",
            extra: {},
          },
          {
            name: "search_result",
            concept: "SearchResult",
            content_type: null,
            preview: null,
            size: null,
            digest: "UVyq5",
            data: {
              answer: "LDdKASaojiqARIDCJSTw",
              sources: [
                {
                  url: "hgyxAfcijBKdPtAmfcha",
                  public_url: "xKNISWXPVVGLTtyBWElJ",
                  mime_type: "ooofYrGnFvYWxgyoqlTJ",
                  filename: "SAPGrYOVYLJFETyEVMqB",
                  title: "UhCicBqerKciCYyjIaGd",
                  snippet: "rOiCMWsvWqZvgDZVIlxq",
                },
                {
                  url: "WhpIemcSTuEFrLEYWZvt",
                  public_url: "kFFrTiFLjDBaKEEzcXXk",
                  mime_type: "fgVXKJzyhNfOltQoyrcr",
                  filename: "LrfoWSxJKHhKvKlyPsXW",
                  title: "AeoPypjpJywBcTxVIhyu",
                  snippet: "ZDAAiGVkZkKoMvxgWVQd",
                },
                {
                  url: "TDdFFqXcvTFIfXiPLUYH",
                  public_url: "vLBOoQgeFvfQROcLULjG",
                  mime_type: "FTBIwcSRHIhcaqTWqQat",
                  filename: "rXTsfCfjKdcNjZPNFApN",
                  title: "BgkfWftmSmtFfKNxNYtZ",
                  snippet: "GrxmpaYeyqasSdUkpxaW",
                },
              ],
            },
            data_text:
              "Search Result:\nLDdKASaojiqARIDCJSTw                                                                                \n\nSources (3):\nUhCicBqerKciCYyjIaGd (hgyxAfcijBKdPtAmfcha)\n  rOiCMWsvWqZvgDZVIlxq\n\nAeoPypjpJywBcTxVIhyu (WhpIemcSTuEFrLEYWZvt)\n  ZDAAiGVkZkKoMvxgWVQd\n\nBgkfWftmSmtFfKNxNYtZ (TDdFFqXcvTFIfXiPLUYH)\n  GrxmpaYeyqasSdUkpxaW\n",
            data_html:
              '<div><p>LDdKASaojiqARIDCJSTw</p><h4>Sources</h4><ul><li><a href="xKNISWXPVVGLTtyBWElJ" class="msg-document">UhCicBqerKciCYyjIaGd</a><br/><small>rOiCMWsvWqZvgDZVIlxq</small></li><li><a href="kFFrTiFLjDBaKEEzcXXk" class="msg-document">AeoPypjpJywBcTxVIhyu</a><br/><small>ZDAAiGVkZkKoMvxgWVQd</small></li><li><a href="vLBOoQgeFvfQROcLULjG" class="msg-document">BgkfWftmSmtFfKNxNYtZ</a><br/><small>GrxmpaYeyqasSdUkpxaW</small></li></ul></div>',
            extra: {},
          },
        ],
        outputs: [
          {
            name: "match",
            concept: "MatchScore",
            content_type: null,
            preview: null,
            size: null,
            digest: "d4ceM",
            data: {
              score: -21601.2201684928,
              strengths: ["b63858c47c"],
              gaps: ["4c07b291e3"],
              recommendation: "QyNVlaFAWBNAVTyTxQeJ",
            },
            data_text:
              " Attribute                        ┃ Value                                       \n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n score                            │ -21601.2201684928                           \n──────────────────────────────────┼─────────────────────────────────────────────\n strengths                        │   1   │ b63858c47c                          \n──────────────────────────────────┼─────────────────────────────────────────────\n gaps                             │   1   │ 4c07b291e3                          \n──────────────────────────────────┼─────────────────────────────────────────────\n recommendation                   │ QyNVlaFAWBNAVTyTxQeJ                        \n",
            data_html:
              "<table><tr><th>score</th><td>-21601.2201684928</td></tr><tr><th>strengths</th><td><ul><li>b63858c47c</li></ul></td></tr><tr><th>gaps</th><td><ul><li>4c07b291e3</li></ul></td></tr><tr><th>recommendation</th><td>QyNVlaFAWBNAVTyTxQeJ</td></tr></table>",
            extra: {},
          },
        ],
      },
      error: null,
      tags: {},
      metrics: {},
      usage: {
        inference_calls: 1,
        rated_inference_calls: 0,
        nb_tokens_by_category: {},
        total_tokens: 0,
        cost: null,
        cost_input: null,
        cost_output: null,
        by_model: [
          {
            inference_model_name: "dry_run",
            inference_model_id: "dry_run",
            model_type: "llm",
            inference_calls: 1,
            rated_inference_calls: 0,
            cost: null,
          },
        ],
        subtree_inference_calls: 1,
        subtree_rated_inference_calls: 0,
        subtree_nb_tokens_by_category: {},
        subtree_total_tokens: 0,
        subtree_cost: null,
        subtree_cost_input: null,
        subtree_cost_output: null,
        subtree_by_model: [
          {
            inference_model_name: "dry_run",
            inference_model_id: "dry_run",
            model_type: "llm",
            inference_calls: 1,
            rated_inference_calls: 0,
            cost: null,
          },
        ],
      },
      execution_data: {
        resolved_model: "claude-4.6-sonnet",
        resolved_model_for_object: "claude-4.6-sonnet",
        is_multiple_output: false,
        rendered_system_prompt: null,
        rendered_user_prompt:
          'Based on this candidate profile and online research, score how well they match our open role:\n\nProfile:\n<profile>\n# name: XQnJeirSOmfSIDuFlwPb\n\n# skills: - c0f1\n\n# experience_years: 4453\n\n# summary: VLSsAMGooTVMzyADqWSJ\n</profile>\n\nOnline research:\n<search_result>\nLDdKASaojiqARIDCJSTw\n\nSources:\n- UhCicBqerKciCYyjIaGd: hgyxAfcijBKdPtAmfcha\n  rOiCMWsvWqZvgDZVIlxq\n- AeoPypjpJywBcTxVIhyu: WhpIemcSTuEFrLEYWZvt\n  ZDAAiGVkZkKoMvxgWVQd\n- BgkfWftmSmtFfKNxNYtZ: TDdFFqXcvTFIfXiPLUYH\n  GrxmpaYeyqasSdUkpxaW\n</search_result>\n\n---\nThe instance we want to generate will be for the following class:\nclass recruitment__MatchScore(StructuredContent):\n    """Score for how well a candidate matches a role"""\n    score: float  # Match score 0-100\n    strengths: List[Any] | None = None  # Key strengths\n    gaps: List[Any] | None = None  # Skill gaps\n    recommendation: str  # Hiring recommendation\n\nDO NOT create information.\nIf some information is not present for an attribute, output the default value or None according to the attribute definition.',
        structuring_path: "object_direct",
      },
    },
    {
      id: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_7",
      kind: "operator",
      pipe_code: "compose_report",
      pipe_type: "PipeCompose",
      description: "Compose final screening report",
      domain_code: "recruitment",
      status: "succeeded",
      skip_reason: null,
      timing: {
        started_at: "2026-08-14T09:49:48.650056Z",
        ended_at: "2026-08-14T09:49:48.670435Z",
        duration: 0.020379,
      },
      io: {
        inputs: [
          {
            name: "profile",
            concept: "CandidateProfile",
            content_type: null,
            preview: null,
            size: null,
            digest: "cW76c",
            data: {
              name: "XQnJeirSOmfSIDuFlwPb",
              skills: ["c0f1"],
              experience_years: 4453,
              summary: "VLSsAMGooTVMzyADqWSJ",
            },
            data_text:
              " Attribute                          ┃ Value                                     \n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n name                               │ XQnJeirSOmfSIDuFlwPb                      \n────────────────────────────────────┼───────────────────────────────────────────\n skills                             │   1   │ c0f1                              \n────────────────────────────────────┼───────────────────────────────────────────\n experience_years                   │ 4453                                      \n────────────────────────────────────┼───────────────────────────────────────────\n summary                            │ VLSsAMGooTVMzyADqWSJ                      \n",
            data_html:
              "<table><tr><th>name</th><td>XQnJeirSOmfSIDuFlwPb</td></tr><tr><th>skills</th><td><ul><li>c0f1</li></ul></td></tr><tr><th>experience_years</th><td>4453</td></tr><tr><th>summary</th><td>VLSsAMGooTVMzyADqWSJ</td></tr></table>",
            extra: {},
          },
          {
            name: "match",
            concept: "MatchScore",
            content_type: null,
            preview: null,
            size: null,
            digest: "d4ceM",
            data: {
              score: -21601.2201684928,
              strengths: ["b63858c47c"],
              gaps: ["4c07b291e3"],
              recommendation: "QyNVlaFAWBNAVTyTxQeJ",
            },
            data_text:
              " Attribute                        ┃ Value                                       \n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╇━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n score                            │ -21601.2201684928                           \n──────────────────────────────────┼─────────────────────────────────────────────\n strengths                        │   1   │ b63858c47c                          \n──────────────────────────────────┼─────────────────────────────────────────────\n gaps                             │   1   │ 4c07b291e3                          \n──────────────────────────────────┼─────────────────────────────────────────────\n recommendation                   │ QyNVlaFAWBNAVTyTxQeJ                        \n",
            data_html:
              "<table><tr><th>score</th><td>-21601.2201684928</td></tr><tr><th>strengths</th><td><ul><li>b63858c47c</li></ul></td></tr><tr><th>gaps</th><td><ul><li>4c07b291e3</li></ul></td></tr><tr><th>recommendation</th><td>QyNVlaFAWBNAVTyTxQeJ</td></tr></table>",
            extra: {},
          },
          {
            name: "card_image",
            concept: "Image",
            content_type: "image/jpeg",
            preview: null,
            size: null,
            digest: "drTUo",
            data: {
              url: "https://storage.googleapis.com/public_test_files_7fa6_4277_9ab/fashion/fashion_photo_1.jpg",
              public_url:
                "https://storage.googleapis.com/public_test_files_7fa6_4277_9ab/fashion/fashion_photo_1.jpg",
              source_prompt:
                "Professional profile card for XQnJeirSOmfSIDuFlwPb, VLSsAMGooTVMzyADqWSJ",
              source_negative_prompt: null,
              caption: null,
              mime_type: "image/jpeg",
              width: 1024,
              height: 1024,
              filename: null,
            },
            data_text:
              "Image:\nURL: https://storage.googleapis.com/public_test_files_7fa6_4277_9ab/fashion/fashion_photo_1.jpg     \nPublic URL: Open Image\nSize: 1024x1024\nMIME Type: image/jpeg\n\nSource Prompt:\nProfessional profile card for XQnJeirSOmfSIDuFlwPb, VLSsAMGooTVMzyADqWSJ\n",
            data_html:
              '<img src="https://storage.googleapis.com/public_test_files_7fa6_4277_9ab/fashion/fashion_photo_1.jpg" class="msg-img">',
            extra: {},
          },
        ],
        outputs: [
          {
            name: "report",
            concept: "Report",
            content_type: null,
            preview: null,
            size: null,
            digest: "fgP57",
            data: {
              text: "# Candidate Screening Report\n\n## Candidate: XQnJeirSOmfSIDuFlwPb\nVLSsAMGooTVMzyADqWSJ\n\n## Match Score: -21601.2201684928/100\nQyNVlaFAWBNAVTyTxQeJ\n\n## Strengths\nmatch.strengths: ```\n['b63858c47c']\n```\n\n## Gaps\nmatch.gaps: ```\n['4c07b291e3']\n```",
            },
            data_text:
              "                                     Candidate Screening Report                                     \n\nCandidate: XQnJeirSOmfSIDuFlwPb                                                                     \n\nVLSsAMGooTVMzyADqWSJ                                                                                \n\nMatch Score: -21601.2201684928/100                                                                  \n\nQyNVlaFAWBNAVTyTxQeJ                                                                                \n\nStrengths                                                                                           \n\nmatch.strengths: ``` ['b63858c47c']                                                                 \n\n                                                                                                    \n                                                                                                    \n ## Gaps                                                                                            \n match.gaps: ```                                                                                    \n ['4c07b291e3']                                                                                     \n                                                                                                    \n",
            data_html:
              "# Candidate Screening Report\n\n## Candidate: XQnJeirSOmfSIDuFlwPb\nVLSsAMGooTVMzyADqWSJ\n\n## Match Score: -21601.2201684928/100\nQyNVlaFAWBNAVTyTxQeJ\n\n## Strengths\nmatch.strengths: ```\n[&#x27;b63858c47c&#x27;]\n```\n\n## Gaps\nmatch.gaps: ```\n[&#x27;4c07b291e3&#x27;]\n```",
            extra: {},
          },
        ],
      },
      error: null,
      tags: {},
      metrics: {},
      usage: {
        inference_calls: 0,
        rated_inference_calls: 0,
        nb_tokens_by_category: {},
        total_tokens: 0,
        cost: null,
        cost_input: null,
        cost_output: null,
        by_model: [],
        subtree_inference_calls: 0,
        subtree_rated_inference_calls: 0,
        subtree_nb_tokens_by_category: {},
        subtree_total_tokens: 0,
        subtree_cost: null,
        subtree_cost_input: null,
        subtree_cost_output: null,
        subtree_by_model: [],
      },
      execution_data: {
        compose_mode: "template",
        rendered_text:
          "# Candidate Screening Report\n\n## Candidate: XQnJeirSOmfSIDuFlwPb\nVLSsAMGooTVMzyADqWSJ\n\n## Match Score: -21601.2201684928/100\nQyNVlaFAWBNAVTyTxQeJ\n\n## Strengths\nmatch.strengths: ```\n['b63858c47c']\n```\n\n## Gaps\nmatch.gaps: ```\n['4c07b291e3']\n```",
      },
    },
  ],
  edges: [
    {
      id: "25618ec2-6c0e-4596-aad2-5f2561785b30:edge_0",
      source: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_0",
      target: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_1",
      kind: "contains",
      optional: false,
      label: null,
      source_stuff_digest: null,
      target_stuff_digest: null,
      meta: {},
    },
    {
      id: "25618ec2-6c0e-4596-aad2-5f2561785b30:edge_1",
      source: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_0",
      target: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_2",
      kind: "contains",
      optional: false,
      label: null,
      source_stuff_digest: null,
      target_stuff_digest: null,
      meta: {},
    },
    {
      id: "25618ec2-6c0e-4596-aad2-5f2561785b30:edge_2",
      source: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_0",
      target: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_3",
      kind: "contains",
      optional: false,
      label: null,
      source_stuff_digest: null,
      target_stuff_digest: null,
      meta: {},
    },
    {
      id: "25618ec2-6c0e-4596-aad2-5f2561785b30:edge_3",
      source: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_3",
      target: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_4",
      kind: "contains",
      optional: false,
      label: null,
      source_stuff_digest: null,
      target_stuff_digest: null,
      meta: {},
    },
    {
      id: "25618ec2-6c0e-4596-aad2-5f2561785b30:edge_4",
      source: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_3",
      target: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_5",
      kind: "contains",
      optional: false,
      label: null,
      source_stuff_digest: null,
      target_stuff_digest: null,
      meta: {},
    },
    {
      id: "25618ec2-6c0e-4596-aad2-5f2561785b30:edge_5",
      source: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_0",
      target: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_6",
      kind: "contains",
      optional: false,
      label: null,
      source_stuff_digest: null,
      target_stuff_digest: null,
      meta: {},
    },
    {
      id: "25618ec2-6c0e-4596-aad2-5f2561785b30:edge_6",
      source: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_0",
      target: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_7",
      kind: "contains",
      optional: false,
      label: null,
      source_stuff_digest: null,
      target_stuff_digest: null,
      meta: {},
    },
    {
      id: "25618ec2-6c0e-4596-aad2-5f2561785b30:asm_edge_0",
      source: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_1",
      target: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_2",
      kind: "data",
      optional: false,
      label: "pages",
      source_stuff_digest: null,
      target_stuff_digest: null,
      meta: {},
    },
    {
      id: "25618ec2-6c0e-4596-aad2-5f2561785b30:asm_edge_1",
      source: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_2",
      target: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_3",
      kind: "data",
      optional: false,
      label: "profile",
      source_stuff_digest: null,
      target_stuff_digest: null,
      meta: {},
    },
    {
      id: "25618ec2-6c0e-4596-aad2-5f2561785b30:asm_edge_2",
      source: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_2",
      target: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_4",
      kind: "data",
      optional: false,
      label: "profile",
      source_stuff_digest: null,
      target_stuff_digest: null,
      meta: {},
    },
    {
      id: "25618ec2-6c0e-4596-aad2-5f2561785b30:asm_edge_3",
      source: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_2",
      target: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_5",
      kind: "data",
      optional: false,
      label: "profile",
      source_stuff_digest: null,
      target_stuff_digest: null,
      meta: {},
    },
    {
      id: "25618ec2-6c0e-4596-aad2-5f2561785b30:asm_edge_4",
      source: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_2",
      target: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_6",
      kind: "data",
      optional: false,
      label: "profile",
      source_stuff_digest: null,
      target_stuff_digest: null,
      meta: {},
    },
    {
      id: "25618ec2-6c0e-4596-aad2-5f2561785b30:asm_edge_5",
      source: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_3",
      target: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_6",
      kind: "data",
      optional: false,
      label: "search_result",
      source_stuff_digest: null,
      target_stuff_digest: null,
      meta: {},
    },
    {
      id: "25618ec2-6c0e-4596-aad2-5f2561785b30:asm_edge_6",
      source: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_2",
      target: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_7",
      kind: "data",
      optional: false,
      label: "profile",
      source_stuff_digest: null,
      target_stuff_digest: null,
      meta: {},
    },
    {
      id: "25618ec2-6c0e-4596-aad2-5f2561785b30:asm_edge_7",
      source: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_6",
      target: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_7",
      kind: "data",
      optional: false,
      label: "match",
      source_stuff_digest: null,
      target_stuff_digest: null,
      meta: {},
    },
    {
      id: "25618ec2-6c0e-4596-aad2-5f2561785b30:asm_edge_8",
      source: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_3",
      target: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_7",
      kind: "data",
      optional: false,
      label: "card_image",
      source_stuff_digest: null,
      target_stuff_digest: null,
      meta: {},
    },
    {
      id: "25618ec2-6c0e-4596-aad2-5f2561785b30:asm_edge_9",
      source: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_4",
      target: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_3",
      kind: "parallel_combine",
      optional: false,
      label: null,
      source_stuff_digest: "UVyq5",
      target_stuff_digest: "98AF9",
      meta: {},
    },
    {
      id: "25618ec2-6c0e-4596-aad2-5f2561785b30:asm_edge_10",
      source: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_5",
      target: "25618ec2-6c0e-4596-aad2-5f2561785b30:node_3",
      kind: "parallel_combine",
      optional: false,
      label: null,
      source_stuff_digest: "drTUo",
      target_stuff_digest: "98AF9",
      meta: {},
    },
  ],
  usage: {
    total: {
      inference_calls: 2,
      rated_inference_calls: 0,
      nb_tokens_by_category: {},
      total_tokens: 0,
      cost: null,
      cost_input: null,
      cost_output: null,
      by_model: [
        {
          inference_model_name: "dry_run",
          inference_model_id: "dry_run",
          model_type: "llm",
          inference_calls: 2,
          rated_inference_calls: 0,
          cost: null,
        },
      ],
      subtree_inference_calls: 2,
      subtree_rated_inference_calls: 0,
      subtree_nb_tokens_by_category: {},
      subtree_total_tokens: 0,
      subtree_cost: null,
      subtree_cost_input: null,
      subtree_cost_output: null,
      subtree_by_model: [
        {
          inference_model_name: "dry_run",
          inference_model_id: "dry_run",
          model_type: "llm",
          inference_calls: 2,
          rated_inference_calls: 0,
          cost: null,
        },
      ],
    },
    unattributed: {
      inference_calls: 0,
      rated_inference_calls: 0,
      nb_tokens_by_category: {},
      total_tokens: 0,
      cost: null,
      cost_input: null,
      cost_output: null,
      by_model: [],
      subtree_inference_calls: 0,
      subtree_rated_inference_calls: 0,
      subtree_nb_tokens_by_category: {},
      subtree_total_tokens: 0,
      subtree_cost: null,
      subtree_cost_input: null,
      subtree_cost_output: null,
      subtree_by_model: [],
    },
  },
  meta: { format: "mthds", mode: "dry" },
  pipe_registry: {
    "recruitment.cv_screening": {
      pipe_category: "PipeController",
      type: "PipeSequence",
      code: "cv_screening",
      domain_code: "recruitment",
      description: "Full CV screening pipeline",
      inputs: {
        cv: {
          concept: {
            code: "Document",
            domain_code: "native",
            description: "A document",
            structure_class_name: "DocumentContent",
            refines: null,
          },
          multiplicity: null,
          presence: "plain",
        },
      },
      output: {
        concept: {
          code: "Report",
          domain_code: "recruitment",
          description: "Formatted screening report",
          structure_class_name: "recruitment__Report",
          refines: "native.Text",
        },
        multiplicity: null,
        presence: "plain",
      },
      sequential_sub_pipes: [
        {
          pipe_code: "extract_cv",
          output_name: "pages",
          output_multiplicity: null,
          batch_params: null,
        },
        {
          pipe_code: "analyze_candidate",
          output_name: "profile",
          output_multiplicity: null,
          batch_params: null,
        },
        {
          pipe_code: "enrich_candidate",
          output_name: "candidate_assets",
          output_multiplicity: null,
          batch_params: null,
        },
        {
          pipe_code: "score_match",
          output_name: "match",
          output_multiplicity: null,
          batch_params: null,
        },
        {
          pipe_code: "compose_report",
          output_name: "report",
          output_multiplicity: null,
          batch_params: null,
        },
      ],
    },
    "recruitment.extract_cv": {
      pipe_category: "PipeOperator",
      type: "PipeExtract",
      code: "extract_cv",
      domain_code: "recruitment",
      description: "Extract pages from CV document",
      inputs: {
        cv: {
          concept: {
            code: "Document",
            domain_code: "native",
            description: "A document",
            structure_class_name: "DocumentContent",
            refines: null,
          },
          multiplicity: null,
          presence: "plain",
        },
      },
      output: {
        concept: {
          code: "Page",
          domain_code: "native",
          description:
            "The content of a page of a document, comprising text and linked images and an optional page view image",
          structure_class_name: "PageContent",
          refines: null,
        },
        multiplicity: true,
        presence: "plain",
      },
      extract_choice: null,
      should_caption_images: false,
      max_page_images: null,
      should_include_page_views: false,
      page_views_dpi: null,
      render_js: null,
      include_raw_html: null,
      image_stuff_name: null,
      document_stuff_name: "cv",
    },
    "recruitment.analyze_candidate": {
      pipe_category: "PipeOperator",
      type: "PipeLLM",
      code: "analyze_candidate",
      domain_code: "recruitment",
      description: "Analyze CV to build candidate profile",
      inputs: {
        pages: {
          concept: {
            code: "Page",
            domain_code: "native",
            description:
              "The content of a page of a document, comprising text and linked images and an optional page view image",
            structure_class_name: "PageContent",
            refines: null,
          },
          multiplicity: true,
          presence: "plain",
        },
      },
      output: {
        concept: {
          code: "CandidateProfile",
          domain_code: "recruitment",
          description: "Structured profile of a job candidate",
          structure_class_name: "recruitment__CandidateProfile",
          refines: null,
        },
        multiplicity: null,
        presence: "plain",
      },
      llm_prompt_spec: {
        system_prompt_blueprint: null,
        prompt_blueprint: {
          template: "Analyze this CV and extract a structured candidate profile:\n\n@pages\n",
          templating_style: null,
          category: "llm_prompt",
          extra_context: null,
        },
        user_image_references: null,
        user_document_references: null,
        system_image_references: null,
        system_document_references: null,
      },
      llm_choices: { for_text: null, for_object: null },
      output_multiplicity: null,
    },
    "recruitment.enrich_candidate": {
      pipe_category: "PipeController",
      type: "PipeParallel",
      code: "enrich_candidate",
      domain_code: "recruitment",
      description: "Search online and generate profile card in parallel",
      inputs: {
        profile: {
          concept: {
            code: "CandidateProfile",
            domain_code: "recruitment",
            description: "Structured profile of a job candidate",
            structure_class_name: "recruitment__CandidateProfile",
            refines: null,
          },
          multiplicity: null,
          presence: "plain",
        },
      },
      output: {
        concept: {
          code: "Composite",
          domain_code: "native",
          description: "A named composition of contents",
          structure_class_name: "CompositeContent",
          refines: null,
        },
        multiplicity: null,
        presence: "plain",
      },
      parallel_sub_pipes: [
        {
          pipe_code: "search_candidate",
          output_name: "search_result",
          output_multiplicity: null,
          batch_params: null,
        },
        {
          pipe_code: "generate_card",
          output_name: "card_image",
          output_multiplicity: null,
          batch_params: null,
        },
      ],
      add_each_output: true,
    },
    "recruitment.search_candidate": {
      pipe_category: "PipeOperator",
      type: "PipeSearch",
      code: "search_candidate",
      domain_code: "recruitment",
      description: "Search for candidate online presence",
      inputs: {
        profile: {
          concept: {
            code: "CandidateProfile",
            domain_code: "recruitment",
            description: "Structured profile of a job candidate",
            structure_class_name: "recruitment__CandidateProfile",
            refines: null,
          },
          multiplicity: null,
          presence: "plain",
        },
      },
      output: {
        concept: {
          code: "SearchResult",
          domain_code: "native",
          description: "A search result with answer and sources",
          structure_class_name: "SearchResultContent",
          refines: null,
        },
        multiplicity: null,
        presence: "plain",
      },
      search_choice: null,
      prompt_blueprint: {
        template: "$profile.name professional background",
        templating_style: null,
        category: "basic",
        extra_context: null,
      },
      include_images_override: null,
      max_results_override: null,
      from_date: null,
      to_date: null,
      include_domains: null,
      exclude_domains: null,
      is_structured_output: false,
    },
    "recruitment.generate_card": {
      pipe_category: "PipeOperator",
      type: "PipeImgGen",
      code: "generate_card",
      domain_code: "recruitment",
      description: "Generate candidate profile card image",
      inputs: {
        profile: {
          concept: {
            code: "CandidateProfile",
            domain_code: "recruitment",
            description: "Structured profile of a job candidate",
            structure_class_name: "recruitment__CandidateProfile",
            refines: null,
          },
          multiplicity: null,
          presence: "plain",
        },
      },
      output: {
        concept: {
          code: "Image",
          domain_code: "native",
          description: "An image",
          structure_class_name: "ImageContent",
          refines: null,
        },
        multiplicity: null,
        presence: "plain",
      },
      img_gen_prompt_blueprint: {
        prompt_blueprint: {
          template: "Professional profile card for $profile.name, $profile.summary",
          templating_style: null,
          category: "img_gen_prompt",
          extra_context: null,
        },
        negative_prompt_blueprint: null,
        image_references: null,
      },
      img_gen_choice: null,
      aspect_ratio: null,
      size: null,
      is_raw: null,
      seed: null,
      background: null,
      output_format: null,
      output_multiplicity: 1,
    },
    "recruitment.score_match": {
      pipe_category: "PipeOperator",
      type: "PipeLLM",
      code: "score_match",
      domain_code: "recruitment",
      description: "Score candidate-role match",
      inputs: {
        profile: {
          concept: {
            code: "CandidateProfile",
            domain_code: "recruitment",
            description: "Structured profile of a job candidate",
            structure_class_name: "recruitment__CandidateProfile",
            refines: null,
          },
          multiplicity: null,
          presence: "plain",
        },
        search_result: {
          concept: {
            code: "SearchResult",
            domain_code: "native",
            description: "A search result with answer and sources",
            structure_class_name: "SearchResultContent",
            refines: null,
          },
          multiplicity: null,
          presence: "plain",
        },
      },
      output: {
        concept: {
          code: "MatchScore",
          domain_code: "recruitment",
          description: "Score for how well a candidate matches a role",
          structure_class_name: "recruitment__MatchScore",
          refines: null,
        },
        multiplicity: null,
        presence: "plain",
      },
      llm_prompt_spec: {
        system_prompt_blueprint: null,
        prompt_blueprint: {
          template:
            "Based on this candidate profile and online research, score how well they match our open role:\n\nProfile:\n@profile\n\nOnline research:\n@search_result\n",
          templating_style: null,
          category: "llm_prompt",
          extra_context: null,
        },
        user_image_references: null,
        user_document_references: null,
        system_image_references: null,
        system_document_references: null,
      },
      llm_choices: { for_text: null, for_object: null },
      output_multiplicity: null,
    },
    "recruitment.compose_report": {
      pipe_category: "PipeOperator",
      type: "PipeCompose",
      code: "compose_report",
      domain_code: "recruitment",
      description: "Compose final screening report",
      inputs: {
        profile: {
          concept: {
            code: "CandidateProfile",
            domain_code: "recruitment",
            description: "Structured profile of a job candidate",
            structure_class_name: "recruitment__CandidateProfile",
            refines: null,
          },
          multiplicity: null,
          presence: "plain",
        },
        match: {
          concept: {
            code: "MatchScore",
            domain_code: "recruitment",
            description: "Score for how well a candidate matches a role",
            structure_class_name: "recruitment__MatchScore",
            refines: null,
          },
          multiplicity: null,
          presence: "plain",
        },
        card_image: {
          concept: {
            code: "Image",
            domain_code: "native",
            description: "An image",
            structure_class_name: "ImageContent",
            refines: null,
          },
          multiplicity: null,
          presence: "plain",
        },
      },
      output: {
        concept: {
          code: "Report",
          domain_code: "recruitment",
          description: "Formatted screening report",
          structure_class_name: "recruitment__Report",
          refines: "native.Text",
        },
        multiplicity: null,
        presence: "plain",
      },
      template:
        "# Candidate Screening Report\n\n## Candidate: $profile.name\n$profile.summary\n\n## Match Score: $match.score/100\n$match.recommendation\n\n## Strengths\n@match.strengths\n\n## Gaps\n@match.gaps\n",
      templating_style: null,
      category: "basic",
      extra_context: null,
      construct_blueprint: null,
    },
  },
  concept_registry: {
    "recruitment.Report": {
      code: "Report",
      domain_code: "recruitment",
      description: "Formatted screening report",
      structure_class_name: "recruitment__Report",
      refines: "native.Text",
      json_schema: {
        description: "Formatted screening report",
        properties: { text: { description: "The text", title: "Text", type: "string" } },
        required: ["text"],
        title: "recruitment__Report",
        type: "object",
      },
    },
    "native.Document": {
      code: "Document",
      domain_code: "native",
      description: "A document",
      structure_class_name: "DocumentContent",
      refines: null,
      json_schema: {
        properties: {
          url: {
            description: "The document URL: a storage URI, an HTTP(S) URL, or a base64 data URL",
            title: "Url",
            type: "string",
          },
          public_url: {
            anyOf: [{ type: "string" }, { type: "null" }],
            default: null,
            description: "The public HTTPS URL of the document",
            title: "Public Url",
          },
          mime_type: {
            anyOf: [{ type: "string" }, { type: "null" }],
            default: null,
            description: "The MIME type of the document",
            title: "Mime Type",
          },
          filename: {
            anyOf: [{ type: "string" }, { type: "null" }],
            default: null,
            description: "The original filename of the document",
            title: "Filename",
          },
          title: {
            anyOf: [{ type: "string" }, { type: "null" }],
            default: null,
            description: "The title of the document or source",
            title: "Title",
          },
          snippet: {
            anyOf: [{ type: "string" }, { type: "null" }],
            default: null,
            description: "A text snippet or excerpt from the document",
            title: "Snippet",
          },
        },
        required: ["url"],
        title: "DocumentContent",
        type: "object",
      },
    },
    "native.Page": {
      code: "Page",
      domain_code: "native",
      description:
        "The content of a page of a document, comprising text and linked images and an optional page view image",
      structure_class_name: "PageContent",
      refines: null,
      json_schema: {
        $defs: {
          ImageContent: {
            properties: {
              url: {
                description: "The image URL: a storage URI, an HTTP(S) URL, or a base64 data URL",
                title: "Url",
                type: "string",
              },
              public_url: {
                anyOf: [{ type: "string" }, { type: "null" }],
                default: null,
                description: "The public URL of the image",
                title: "Public Url",
              },
              source_prompt: {
                anyOf: [{ type: "string" }, { type: "null" }],
                default: null,
                description: "The source prompt of the image",
                title: "Source Prompt",
              },
              source_negative_prompt: {
                anyOf: [{ type: "string" }, { type: "null" }],
                default: null,
                description: "The source negative prompt of the image",
                title: "Source Negative Prompt",
              },
              caption: {
                anyOf: [{ type: "string" }, { type: "null" }],
                default: null,
                description: "The caption of the image",
                title: "Caption",
              },
              mime_type: {
                anyOf: [{ type: "string" }, { type: "null" }],
                default: null,
                description: "The MIME type of the image",
                title: "Mime Type",
              },
              width: {
                anyOf: [{ exclusiveMinimum: 0, type: "integer" }, { type: "null" }],
                default: null,
                description: "The width of the image, in pixels",
                title: "Width",
              },
              height: {
                anyOf: [{ exclusiveMinimum: 0, type: "integer" }, { type: "null" }],
                default: null,
                description: "The height of the image, in pixels",
                title: "Height",
              },
              filename: {
                anyOf: [{ type: "string" }, { type: "null" }],
                default: null,
                description: "The original filename of the image",
                title: "Filename",
              },
            },
            required: ["url"],
            title: "ImageContent",
            type: "object",
          },
          TextAndImagesContent: {
            properties: {
              text: {
                anyOf: [{ $ref: "#/$defs/TextContent" }, { type: "null" }],
                default: null,
                description: "A text content",
              },
              images: {
                anyOf: [
                  { items: { $ref: "#/$defs/ImageContent" }, type: "array" },
                  { type: "null" },
                ],
                default: null,
                description: "A list of images that were extracted from the text",
                title: "Images",
              },
              raw_html: {
                anyOf: [{ type: "string" }, { type: "null" }],
                default: null,
                description: "The raw HTML of the fetched page, if requested",
                title: "Raw Html",
              },
            },
            title: "TextAndImagesContent",
            type: "object",
          },
          TextContent: {
            properties: { text: { description: "The text", title: "Text", type: "string" } },
            required: ["text"],
            title: "TextContent",
            type: "object",
          },
        },
        properties: {
          text_and_images: {
            $ref: "#/$defs/TextAndImagesContent",
            description: "The text and images content extracted from the page",
          },
          page_view: {
            anyOf: [{ $ref: "#/$defs/ImageContent" }, { type: "null" }],
            default: null,
            description: "The screenshot of the page",
          },
        },
        required: ["text_and_images"],
        title: "PageContent",
        type: "object",
      },
    },
    "recruitment.CandidateProfile": {
      code: "CandidateProfile",
      domain_code: "recruitment",
      description: "Structured profile of a job candidate",
      structure_class_name: "recruitment__CandidateProfile",
      refines: null,
      json_schema: {
        description: "Structured profile of a job candidate",
        properties: {
          name: { description: "Full name", title: "Name", type: "string" },
          skills: {
            anyOf: [{ items: {}, type: "array" }, { type: "null" }],
            default: null,
            description: "Key skills",
            title: "Skills",
          },
          experience_years: {
            anyOf: [{ type: "integer" }, { type: "null" }],
            default: null,
            description: "Years of experience",
            title: "Experience Years",
          },
          summary: { description: "Brief profile summary", title: "Summary", type: "string" },
        },
        required: ["name", "summary"],
        title: "recruitment__CandidateProfile",
        type: "object",
      },
    },
    "native.Composite": {
      code: "Composite",
      domain_code: "native",
      description: "A named composition of contents",
      structure_class_name: "CompositeContent",
      refines: null,
      json_schema: {
        additionalProperties: true,
        description:
          'Untyped named composition of contents — the structure class of `native.Composite`.\n\nHolds its named sub-contents as top-level fields (pydantic `extra="allow"`), so the\nserialized shape matches a bespoke structured concept: `{name: content, ...}` with no\nwrapper key. This is the combination vehicle for a `PipeParallel` whose author does not\nwant to declare a bespoke concept: each branch\'s `result` name becomes a field.',
        properties: {},
        title: "CompositeContent",
        type: "object",
      },
    },
    "native.SearchResult": {
      code: "SearchResult",
      domain_code: "native",
      description: "A search result with answer and sources",
      structure_class_name: "SearchResultContent",
      refines: null,
      json_schema: {
        $defs: {
          DocumentContent: {
            properties: {
              url: {
                description:
                  "The document URL: a storage URI, an HTTP(S) URL, or a base64 data URL",
                title: "Url",
                type: "string",
              },
              public_url: {
                anyOf: [{ type: "string" }, { type: "null" }],
                default: null,
                description: "The public HTTPS URL of the document",
                title: "Public Url",
              },
              mime_type: {
                anyOf: [{ type: "string" }, { type: "null" }],
                default: null,
                description: "The MIME type of the document",
                title: "Mime Type",
              },
              filename: {
                anyOf: [{ type: "string" }, { type: "null" }],
                default: null,
                description: "The original filename of the document",
                title: "Filename",
              },
              title: {
                anyOf: [{ type: "string" }, { type: "null" }],
                default: null,
                description: "The title of the document or source",
                title: "Title",
              },
              snippet: {
                anyOf: [{ type: "string" }, { type: "null" }],
                default: null,
                description: "A text snippet or excerpt from the document",
                title: "Snippet",
              },
            },
            required: ["url"],
            title: "DocumentContent",
            type: "object",
          },
        },
        description: "Represents the result of a search query with an answer and list of sources.",
        properties: {
          answer: {
            description: "The answer to the search query",
            title: "Answer",
            type: "string",
          },
          sources: {
            description: "The source documents supporting the answer",
            items: { $ref: "#/$defs/DocumentContent" },
            title: "Sources",
            type: "array",
          },
        },
        required: ["answer"],
        title: "SearchResultContent",
        type: "object",
      },
    },
    "native.Image": {
      code: "Image",
      domain_code: "native",
      description: "An image",
      structure_class_name: "ImageContent",
      refines: null,
      json_schema: {
        properties: {
          url: {
            description: "The image URL: a storage URI, an HTTP(S) URL, or a base64 data URL",
            title: "Url",
            type: "string",
          },
          public_url: {
            anyOf: [{ type: "string" }, { type: "null" }],
            default: null,
            description: "The public URL of the image",
            title: "Public Url",
          },
          source_prompt: {
            anyOf: [{ type: "string" }, { type: "null" }],
            default: null,
            description: "The source prompt of the image",
            title: "Source Prompt",
          },
          source_negative_prompt: {
            anyOf: [{ type: "string" }, { type: "null" }],
            default: null,
            description: "The source negative prompt of the image",
            title: "Source Negative Prompt",
          },
          caption: {
            anyOf: [{ type: "string" }, { type: "null" }],
            default: null,
            description: "The caption of the image",
            title: "Caption",
          },
          mime_type: {
            anyOf: [{ type: "string" }, { type: "null" }],
            default: null,
            description: "The MIME type of the image",
            title: "Mime Type",
          },
          width: {
            anyOf: [{ exclusiveMinimum: 0, type: "integer" }, { type: "null" }],
            default: null,
            description: "The width of the image, in pixels",
            title: "Width",
          },
          height: {
            anyOf: [{ exclusiveMinimum: 0, type: "integer" }, { type: "null" }],
            default: null,
            description: "The height of the image, in pixels",
            title: "Height",
          },
          filename: {
            anyOf: [{ type: "string" }, { type: "null" }],
            default: null,
            description: "The original filename of the image",
            title: "Filename",
          },
        },
        required: ["url"],
        title: "ImageContent",
        type: "object",
      },
    },
    "recruitment.MatchScore": {
      code: "MatchScore",
      domain_code: "recruitment",
      description: "Score for how well a candidate matches a role",
      structure_class_name: "recruitment__MatchScore",
      refines: null,
      json_schema: {
        description: "Score for how well a candidate matches a role",
        properties: {
          score: { description: "Match score 0-100", title: "Score", type: "number" },
          strengths: {
            anyOf: [{ items: {}, type: "array" }, { type: "null" }],
            default: null,
            description: "Key strengths",
            title: "Strengths",
          },
          gaps: {
            anyOf: [{ items: {}, type: "array" }, { type: "null" }],
            default: null,
            description: "Skill gaps",
            title: "Gaps",
          },
          recommendation: {
            description: "Hiring recommendation",
            title: "Recommendation",
            type: "string",
          },
        },
        required: ["score", "recommendation"],
        title: "recruitment__MatchScore",
        type: "object",
      },
    },
  },
} as unknown as GraphSpec;
