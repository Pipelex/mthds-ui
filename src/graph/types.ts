 ...
 * the container class. `getPaletteForTheme` takes this. `auto` is never a
 ...
 * and reported. `auto` follows the host environment (browser `prefers-color-scheme`
...
export interface GraphConfig {
  ...
  foldMode?: FoldMode;
  /**
   * Theme *mode*: `dark | light | auto`. `auto` follows the host environment.
   * Defaults to `auto` (see `DEFAULT_GRAPH_CONFIG`). Pass `dark`/`light` to pin
   * a fixed appearance.
   */
