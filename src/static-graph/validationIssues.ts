 ...
 * - A declaration path `pipe.<code>[...]` → `pipeCode`.
 ...
 */
function targetFromPath(path: string | undefined): Pick<ValidationIssue, "pipeCode" | "nodeId"> {
  if (!path) return {};
  if (path.includes("/")) return { nodeId: path };
  if (path.startsWith("pipe.")) {
    const code = path.split(".")[1];
    if (code) return { pipeCode: code };
  }
  return {};
}
 ...
 * targeting fields (`pipeCode` / `nodeId`) are filled so the viewer can
...
export function staticDiagnosticsToValidationIssues(diagnostics: Diagnostic[]): ValidationIssue[] {
  return diagnostics.map((diagnostic) => ({
    ...
    origin: "static",
    ...targetFromPath(diagnostic.path),
  }));
}
