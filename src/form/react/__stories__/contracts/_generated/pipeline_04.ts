/**
 * Auto-generated by scripts/generate-fixtures.mjs — this bundle's three
 * validate views, each keyed by namespaced pipe_ref. DO NOT EDIT.
 * Regenerate with `make fixtures-contracts`.
 */
import type { InputForm, OutputForm, PipeIOContracts } from "@pipelex/mthds-form";

export const CONTRACTS_LONG_SEQUENCE = {
  "data_pipeline.build_index": {
    inputs: {
      embeddings: {
        concept_ref: "data_pipeline.Embedding",
        item_count: null,
        json_schema: {
          items: {
            description: "Vector embedding of text",
            properties: {
              text: {
                description: "The text",
                title: "Text",
                type: "string",
              },
            },
            required: ["text"],
            title: "data_pipeline.Embedding",
            type: "object",
          },
          type: "array",
        },
        multiplicity: "variable",
        presence: "plain",
      },
    },
    output: {
      concept_ref: "data_pipeline.VectorIndex",
      item_count: null,
      json_schema: {
        description: "An indexed collection of vectors",
        properties: {
          json_obj: {
            additionalProperties: true,
            description: "The JSON object",
            title: "Json Obj",
            type: "object",
          },
        },
        required: ["json_obj"],
        title: "data_pipeline.VectorIndex",
        type: "object",
      },
      multiplicity: "single",
      optional: false,
    },
  },
  "data_pipeline.chunk_text": {
    inputs: {
      clean_text: {
        concept_ref: "data_pipeline.CleanText",
        item_count: null,
        json_schema: {
          description: "Text that has been cleaned and normalized",
          properties: {
            text: {
              description: "The text",
              title: "Text",
              type: "string",
            },
          },
          required: ["text"],
          title: "data_pipeline.CleanText",
          type: "object",
        },
        multiplicity: "single",
        presence: "plain",
      },
    },
    output: {
      concept_ref: "data_pipeline.TextChunk",
      item_count: null,
      json_schema: {
        $defs: {
          data_pipeline__TextChunk: {
            description: "A segment of text split for processing",
            properties: {
              text: {
                description: "The text",
                title: "Text",
                type: "string",
              },
            },
            required: ["text"],
            title: "data_pipeline__TextChunk",
            type: "object",
          },
        },
        properties: {
          items: {
            items: {
              $ref: "#/$defs/data_pipeline__TextChunk",
            },
            title: "Items",
            type: "array",
          },
        },
        required: ["items"],
        title: "ListContent[data_pipeline__TextChunk]",
        type: "object",
      },
      multiplicity: "variable",
      optional: false,
    },
  },
  "data_pipeline.clean_text": {
    inputs: {
      pages: {
        concept_ref: "native.Page",
        item_count: null,
        json_schema: {
          items: {
            $defs: {
              ImageContent: {
                description: "An image",
                properties: {
                  caption: {
                    anyOf: [
                      {
                        type: "string",
                      },
                      {
                        type: "null",
                      },
                    ],
                    default: null,
                    description: "The caption of the image",
                    title: "Caption",
                  },
                  filename: {
                    anyOf: [
                      {
                        type: "string",
                      },
                      {
                        type: "null",
                      },
                    ],
                    default: null,
                    description: "The original filename of the image",
                    title: "Filename",
                  },
                  height: {
                    anyOf: [
                      {
                        exclusiveMinimum: 0,
                        type: "integer",
                      },
                      {
                        type: "null",
                      },
                    ],
                    default: null,
                    description: "The height of the image, in pixels",
                    title: "Height",
                  },
                  mime_type: {
                    anyOf: [
                      {
                        type: "string",
                      },
                      {
                        type: "null",
                      },
                    ],
                    default: null,
                    description: "The MIME type of the image",
                    title: "Mime Type",
                  },
                  public_url: {
                    anyOf: [
                      {
                        type: "string",
                      },
                      {
                        type: "null",
                      },
                    ],
                    default: null,
                    description: "The public URL of the image",
                    title: "Public Url",
                  },
                  source_negative_prompt: {
                    anyOf: [
                      {
                        type: "string",
                      },
                      {
                        type: "null",
                      },
                    ],
                    default: null,
                    description: "The source negative prompt of the image",
                    title: "Source Negative Prompt",
                  },
                  source_prompt: {
                    anyOf: [
                      {
                        type: "string",
                      },
                      {
                        type: "null",
                      },
                    ],
                    default: null,
                    description: "The source prompt of the image",
                    title: "Source Prompt",
                  },
                  url: {
                    description:
                      "The image URL: a storage URI, an HTTP(S) URL, or a base64 data URL",
                    title: "Url",
                    type: "string",
                  },
                  width: {
                    anyOf: [
                      {
                        exclusiveMinimum: 0,
                        type: "integer",
                      },
                      {
                        type: "null",
                      },
                    ],
                    default: null,
                    description: "The width of the image, in pixels",
                    title: "Width",
                  },
                },
                required: ["url"],
                title: "ImageContent",
                type: "object",
              },
              TextAndImagesContent: {
                properties: {
                  images: {
                    anyOf: [
                      {
                        items: {
                          $ref: "#/$defs/ImageContent",
                        },
                        type: "array",
                      },
                      {
                        type: "null",
                      },
                    ],
                    default: null,
                    description: "A list of images that were extracted from the text",
                    title: "Images",
                  },
                  raw_html: {
                    anyOf: [
                      {
                        type: "string",
                      },
                      {
                        type: "null",
                      },
                    ],
                    default: null,
                    description: "The raw HTML of the fetched page, if requested",
                    title: "Raw Html",
                  },
                  text: {
                    anyOf: [
                      {
                        $ref: "#/$defs/TextContent",
                      },
                      {
                        type: "null",
                      },
                    ],
                    default: null,
                    description: "A text content",
                  },
                },
                title: "TextAndImagesContent",
                type: "object",
              },
              TextContent: {
                description: "A text",
                properties: {
                  text: {
                    description: "The text",
                    title: "Text",
                    type: "string",
                  },
                },
                required: ["text"],
                title: "TextContent",
                type: "object",
              },
            },
            description:
              "The content of a page of a document, comprising text and linked images and an optional page view image",
            properties: {
              page_view: {
                anyOf: [
                  {
                    $ref: "#/$defs/ImageContent",
                  },
                  {
                    type: "null",
                  },
                ],
                default: null,
                description: "The screenshot of the page",
              },
              text_and_images: {
                $ref: "#/$defs/TextAndImagesContent",
                description: "The text and images content extracted from the page",
              },
            },
            required: ["text_and_images"],
            title: "native.Page",
            type: "object",
          },
          type: "array",
        },
        multiplicity: "variable",
        presence: "plain",
      },
    },
    output: {
      concept_ref: "data_pipeline.CleanText",
      item_count: null,
      json_schema: {
        description: "Text that has been cleaned and normalized",
        properties: {
          text: {
            description: "The text",
            title: "Text",
            type: "string",
          },
        },
        required: ["text"],
        title: "data_pipeline.CleanText",
        type: "object",
      },
      multiplicity: "single",
      optional: false,
    },
  },
  "data_pipeline.embed_chunks": {
    inputs: {
      chunks: {
        concept_ref: "data_pipeline.TextChunk",
        item_count: null,
        json_schema: {
          items: {
            description: "A segment of text split for processing",
            properties: {
              text: {
                description: "The text",
                title: "Text",
                type: "string",
              },
            },
            required: ["text"],
            title: "data_pipeline.TextChunk",
            type: "object",
          },
          type: "array",
        },
        multiplicity: "variable",
        presence: "plain",
      },
    },
    output: {
      concept_ref: "data_pipeline.Embedding",
      item_count: null,
      json_schema: {
        $defs: {
          data_pipeline__Embedding: {
            description: "Vector embedding of text",
            properties: {
              text: {
                description: "The text",
                title: "Text",
                type: "string",
              },
            },
            required: ["text"],
            title: "data_pipeline__Embedding",
            type: "object",
          },
        },
        properties: {
          items: {
            items: {
              $ref: "#/$defs/data_pipeline__Embedding",
            },
            title: "Items",
            type: "array",
          },
        },
        required: ["items"],
        title: "ListContent[data_pipeline__Embedding]",
        type: "object",
      },
      multiplicity: "variable",
      optional: false,
    },
  },
  "data_pipeline.extract_raw": {
    inputs: {
      document: {
        concept_ref: "native.Document",
        item_count: null,
        json_schema: {
          description: "A document",
          properties: {
            filename: {
              anyOf: [
                {
                  type: "string",
                },
                {
                  type: "null",
                },
              ],
              default: null,
              description: "The original filename of the document",
              title: "Filename",
            },
            mime_type: {
              anyOf: [
                {
                  type: "string",
                },
                {
                  type: "null",
                },
              ],
              default: null,
              description: "The MIME type of the document",
              title: "Mime Type",
            },
            public_url: {
              anyOf: [
                {
                  type: "string",
                },
                {
                  type: "null",
                },
              ],
              default: null,
              description: "The public HTTPS URL of the document",
              title: "Public Url",
            },
            snippet: {
              anyOf: [
                {
                  type: "string",
                },
                {
                  type: "null",
                },
              ],
              default: null,
              description: "A text snippet or excerpt from the document",
              title: "Snippet",
            },
            title: {
              anyOf: [
                {
                  type: "string",
                },
                {
                  type: "null",
                },
              ],
              default: null,
              description: "The title of the document or source",
              title: "Title",
            },
            url: {
              description: "The document URL: a storage URI, an HTTP(S) URL, or a base64 data URL",
              title: "Url",
              type: "string",
            },
          },
          required: ["url"],
          title: "native.Document",
          type: "object",
        },
        multiplicity: "single",
        presence: "plain",
      },
    },
    output: {
      concept_ref: "native.Page",
      item_count: null,
      json_schema: {
        $defs: {
          ImageContent: {
            description: "An image",
            properties: {
              caption: {
                anyOf: [
                  {
                    type: "string",
                  },
                  {
                    type: "null",
                  },
                ],
                default: null,
                description: "The caption of the image",
                title: "Caption",
              },
              filename: {
                anyOf: [
                  {
                    type: "string",
                  },
                  {
                    type: "null",
                  },
                ],
                default: null,
                description: "The original filename of the image",
                title: "Filename",
              },
              height: {
                anyOf: [
                  {
                    exclusiveMinimum: 0,
                    type: "integer",
                  },
                  {
                    type: "null",
                  },
                ],
                default: null,
                description: "The height of the image, in pixels",
                title: "Height",
              },
              mime_type: {
                anyOf: [
                  {
                    type: "string",
                  },
                  {
                    type: "null",
                  },
                ],
                default: null,
                description: "The MIME type of the image",
                title: "Mime Type",
              },
              public_url: {
                anyOf: [
                  {
                    type: "string",
                  },
                  {
                    type: "null",
                  },
                ],
                default: null,
                description: "The public URL of the image",
                title: "Public Url",
              },
              source_negative_prompt: {
                anyOf: [
                  {
                    type: "string",
                  },
                  {
                    type: "null",
                  },
                ],
                default: null,
                description: "The source negative prompt of the image",
                title: "Source Negative Prompt",
              },
              source_prompt: {
                anyOf: [
                  {
                    type: "string",
                  },
                  {
                    type: "null",
                  },
                ],
                default: null,
                description: "The source prompt of the image",
                title: "Source Prompt",
              },
              url: {
                description: "The image URL: a storage URI, an HTTP(S) URL, or a base64 data URL",
                title: "Url",
                type: "string",
              },
              width: {
                anyOf: [
                  {
                    exclusiveMinimum: 0,
                    type: "integer",
                  },
                  {
                    type: "null",
                  },
                ],
                default: null,
                description: "The width of the image, in pixels",
                title: "Width",
              },
            },
            required: ["url"],
            title: "ImageContent",
            type: "object",
          },
          PageContent: {
            description:
              "The content of a page of a document, comprising text and linked images and an optional page view image",
            properties: {
              page_view: {
                anyOf: [
                  {
                    $ref: "#/$defs/ImageContent",
                  },
                  {
                    type: "null",
                  },
                ],
                default: null,
                description: "The screenshot of the page",
              },
              text_and_images: {
                $ref: "#/$defs/TextAndImagesContent",
                description: "The text and images content extracted from the page",
              },
            },
            required: ["text_and_images"],
            title: "PageContent",
            type: "object",
          },
          TextAndImagesContent: {
            properties: {
              images: {
                anyOf: [
                  {
                    items: {
                      $ref: "#/$defs/ImageContent",
                    },
                    type: "array",
                  },
                  {
                    type: "null",
                  },
                ],
                default: null,
                description: "A list of images that were extracted from the text",
                title: "Images",
              },
              raw_html: {
                anyOf: [
                  {
                    type: "string",
                  },
                  {
                    type: "null",
                  },
                ],
                default: null,
                description: "The raw HTML of the fetched page, if requested",
                title: "Raw Html",
              },
              text: {
                anyOf: [
                  {
                    $ref: "#/$defs/TextContent",
                  },
                  {
                    type: "null",
                  },
                ],
                default: null,
                description: "A text content",
              },
            },
            title: "TextAndImagesContent",
            type: "object",
          },
          TextContent: {
            description: "A text",
            properties: {
              text: {
                description: "The text",
                title: "Text",
                type: "string",
              },
            },
            required: ["text"],
            title: "TextContent",
            type: "object",
          },
        },
        properties: {
          items: {
            items: {
              $ref: "#/$defs/PageContent",
            },
            title: "Items",
            type: "array",
          },
        },
        required: ["items"],
        title: "ListContent[PageContent]",
        type: "object",
      },
      multiplicity: "variable",
      optional: false,
    },
  },
  "data_pipeline.ingest_pipeline": {
    inputs: {
      document: {
        concept_ref: "native.Document",
        item_count: null,
        json_schema: {
          description: "A document",
          properties: {
            filename: {
              anyOf: [
                {
                  type: "string",
                },
                {
                  type: "null",
                },
              ],
              default: null,
              description: "The original filename of the document",
              title: "Filename",
            },
            mime_type: {
              anyOf: [
                {
                  type: "string",
                },
                {
                  type: "null",
                },
              ],
              default: null,
              description: "The MIME type of the document",
              title: "Mime Type",
            },
            public_url: {
              anyOf: [
                {
                  type: "string",
                },
                {
                  type: "null",
                },
              ],
              default: null,
              description: "The public HTTPS URL of the document",
              title: "Public Url",
            },
            snippet: {
              anyOf: [
                {
                  type: "string",
                },
                {
                  type: "null",
                },
              ],
              default: null,
              description: "A text snippet or excerpt from the document",
              title: "Snippet",
            },
            title: {
              anyOf: [
                {
                  type: "string",
                },
                {
                  type: "null",
                },
              ],
              default: null,
              description: "The title of the document or source",
              title: "Title",
            },
            url: {
              description: "The document URL: a storage URI, an HTTP(S) URL, or a base64 data URL",
              title: "Url",
              type: "string",
            },
          },
          required: ["url"],
          title: "native.Document",
          type: "object",
        },
        multiplicity: "single",
        presence: "plain",
      },
    },
    output: {
      concept_ref: "data_pipeline.ValidationReport",
      item_count: null,
      json_schema: {
        description: "Report validating the index quality",
        properties: {
          text: {
            description: "The text",
            title: "Text",
            type: "string",
          },
        },
        required: ["text"],
        title: "data_pipeline.ValidationReport",
        type: "object",
      },
      multiplicity: "single",
      optional: false,
    },
  },
  "data_pipeline.validate_index": {
    inputs: {
      index: {
        concept_ref: "data_pipeline.VectorIndex",
        item_count: null,
        json_schema: {
          description: "An indexed collection of vectors",
          properties: {
            json_obj: {
              additionalProperties: true,
              description: "The JSON object",
              title: "Json Obj",
              type: "object",
            },
          },
          required: ["json_obj"],
          title: "data_pipeline.VectorIndex",
          type: "object",
        },
        multiplicity: "single",
        presence: "plain",
      },
    },
    output: {
      concept_ref: "data_pipeline.ValidationReport",
      item_count: null,
      json_schema: {
        description: "Report validating the index quality",
        properties: {
          text: {
            description: "The text",
            title: "Text",
            type: "string",
          },
        },
        required: ["text"],
        title: "data_pipeline.ValidationReport",
        type: "object",
      },
      multiplicity: "single",
      optional: false,
    },
  },
} as unknown as PipeIOContracts;

export const INPUT_FORM_LONG_SEQUENCE = {
  "data_pipeline.build_index": {
    fields: [
      {
        concept_ref: "data_pipeline.Embedding",
        description: "Vector embedding of text",
        gating: false,
        item: {
          concept_ref: "data_pipeline.Embedding",
          description: "Vector embedding of text",
          kind: "prose",
          refines: ["native.Text"],
          required: true,
        },
        kind: "list",
        name: "embeddings",
        presence: "plain",
        refines: ["native.Text"],
        required: true,
      },
    ],
  },
  "data_pipeline.chunk_text": {
    fields: [
      {
        concept_ref: "data_pipeline.CleanText",
        description: "Text that has been cleaned and normalized",
        gating: true,
        kind: "prose",
        name: "clean_text",
        presence: "plain",
        refines: ["native.Text"],
        required: true,
      },
    ],
  },
  "data_pipeline.clean_text": {
    fields: [
      {
        concept_ref: "native.Page",
        description:
          "The content of a page of a document, comprising text and linked images and an optional page view image",
        gating: false,
        item: {
          concept_ref: "native.Page",
          description:
            "The content of a page of a document, comprising text and linked images and an optional page view image",
          fields: [
            {
              concept_ref: "native.TextAndImages",
              description: "The text and images content extracted from the page",
              fields: [
                {
                  concept_ref: "native.Text",
                  description: "A text content",
                  kind: "prose",
                  name: "text",
                  required: false,
                },
                {
                  concept_ref: "native.Image",
                  description: "A list of images that were extracted from the text",
                  item: {
                    concept_ref: "native.Image",
                    description: "An image",
                    kind: "image",
                    required: true,
                  },
                  kind: "list",
                  name: "images",
                  required: false,
                },
                {
                  description: "The raw HTML of the fetched page, if requested",
                  kind: "text",
                  name: "raw_html",
                  required: false,
                },
              ],
              kind: "object",
              name: "text_and_images",
              required: true,
            },
            {
              concept_ref: "native.Image",
              description: "The screenshot of the page",
              kind: "image",
              name: "page_view",
              required: false,
            },
          ],
          kind: "object",
          required: true,
        },
        kind: "list",
        name: "pages",
        presence: "plain",
        required: true,
      },
    ],
  },
  "data_pipeline.embed_chunks": {
    fields: [
      {
        concept_ref: "data_pipeline.TextChunk",
        description: "A segment of text split for processing",
        gating: false,
        item: {
          concept_ref: "data_pipeline.TextChunk",
          description: "A segment of text split for processing",
          kind: "prose",
          refines: ["native.Text"],
          required: true,
        },
        kind: "list",
        name: "chunks",
        presence: "plain",
        refines: ["native.Text"],
        required: true,
      },
    ],
  },
  "data_pipeline.extract_raw": {
    fields: [
      {
        concept_ref: "native.Document",
        description: "A document",
        gating: true,
        kind: "document",
        name: "document",
        presence: "plain",
        required: true,
      },
    ],
  },
  "data_pipeline.ingest_pipeline": {
    fields: [
      {
        concept_ref: "native.Document",
        description: "A document",
        gating: true,
        kind: "document",
        name: "document",
        presence: "plain",
        required: true,
      },
    ],
  },
  "data_pipeline.validate_index": {
    fields: [
      {
        concept_ref: "data_pipeline.VectorIndex",
        description: "An indexed collection of vectors",
        gating: true,
        kind: "unknown",
        name: "index",
        presence: "plain",
        refines: ["native.JSON"],
        required: true,
      },
    ],
  },
} as unknown as InputForm;

export const OUTPUT_FORM_LONG_SEQUENCE = {
  "data_pipeline.build_index": {
    field: {
      concept_ref: "data_pipeline.VectorIndex",
      description: "An indexed collection of vectors",
      kind: "unknown",
      name: "output",
      refines: ["native.JSON"],
      required: true,
    },
  },
  "data_pipeline.chunk_text": {
    field: {
      concept_ref: "data_pipeline.TextChunk",
      description: "A segment of text split for processing",
      item: {
        concept_ref: "data_pipeline.TextChunk",
        description: "A segment of text split for processing",
        kind: "prose",
        refines: ["native.Text"],
        required: true,
      },
      kind: "list",
      name: "output",
      refines: ["native.Text"],
      required: true,
    },
  },
  "data_pipeline.clean_text": {
    field: {
      concept_ref: "data_pipeline.CleanText",
      description: "Text that has been cleaned and normalized",
      kind: "prose",
      name: "output",
      refines: ["native.Text"],
      required: true,
    },
  },
  "data_pipeline.embed_chunks": {
    field: {
      concept_ref: "data_pipeline.Embedding",
      description: "Vector embedding of text",
      item: {
        concept_ref: "data_pipeline.Embedding",
        description: "Vector embedding of text",
        kind: "prose",
        refines: ["native.Text"],
        required: true,
      },
      kind: "list",
      name: "output",
      refines: ["native.Text"],
      required: true,
    },
  },
  "data_pipeline.extract_raw": {
    field: {
      concept_ref: "native.Page",
      description:
        "The content of a page of a document, comprising text and linked images and an optional page view image",
      item: {
        concept_ref: "native.Page",
        description:
          "The content of a page of a document, comprising text and linked images and an optional page view image",
        fields: [
          {
            concept_ref: "native.TextAndImages",
            description: "The text and images content extracted from the page",
            fields: [
              {
                concept_ref: "native.Text",
                description: "A text content",
                kind: "prose",
                name: "text",
                required: false,
              },
              {
                concept_ref: "native.Image",
                description: "A list of images that were extracted from the text",
                item: {
                  concept_ref: "native.Image",
                  description: "An image",
                  kind: "image",
                  required: true,
                },
                kind: "list",
                name: "images",
                required: false,
              },
              {
                description: "The raw HTML of the fetched page, if requested",
                kind: "text",
                name: "raw_html",
                required: false,
              },
            ],
            kind: "object",
            name: "text_and_images",
            required: true,
          },
          {
            concept_ref: "native.Image",
            description: "The screenshot of the page",
            kind: "image",
            name: "page_view",
            required: false,
          },
        ],
        kind: "object",
        required: true,
      },
      kind: "list",
      name: "output",
      required: true,
    },
  },
  "data_pipeline.ingest_pipeline": {
    field: {
      concept_ref: "data_pipeline.ValidationReport",
      description: "Report validating the index quality",
      kind: "prose",
      name: "output",
      refines: ["native.Text"],
      required: true,
    },
  },
  "data_pipeline.validate_index": {
    field: {
      concept_ref: "data_pipeline.ValidationReport",
      description: "Report validating the index quality",
      kind: "prose",
      name: "output",
      refines: ["native.Text"],
      required: true,
    },
  },
} as unknown as OutputForm;
