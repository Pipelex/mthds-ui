...
- **Fixed (Greptile, PR #65) — the corpus oracle validated only the first occurrence of each code.** Legitimate, and more than theoretical here: fixtures are regenerated per-pipeline, so the committed corpus is a **mix of pipelex versions**, and a rewording between versions would appear only in the pipelines regenerated since. Taking the first occurrence would validate whichever pipeline sorts first and miss the rest — `Text` alone appears in 19 fixtures. Now validates every occurrence, compared per pipeline so a failure names the fixture. Negative-tested by perturbing `native.Page` in `pipeline_31` alone. No divergence exists in the corpus today; all 14 codes have a single distinct variant.
...
### PR #65 — bot review round

CI green (Quality Checks, CLAAssistant, gate-release; Greptile pass). Five inline comments from Greptile, cubic, and Codex, deduplicated to four findings. Each was verified against the code rather than taken at face value; all threads replied to and resolved.

- **Greptile + Codex, same finding (P2), already fixed** — the corpus oracle validated only the first occurrence per code. Fixed in `73c3737` before Codex's copy arrived; both reviewed `bd08be8`.
- **cubic (P3) — `pipeline_34`'s live fixture records `number: 37` for a 28-word brief.** Arithmetic correct, impact nil: `count_words` is a `PipeLLM` asking a model to count words, and the LIVE fixture's job is to record what the model actually returned. Nothing asserts the value (`structuralFingerprint` captures ids/types/edges, never payloads), and Workflow Rule 7 forbids hand-editing generated fixtures anyway. Dismissed.
- **cubic (P3) — the Phase 4 checklist "contradicts" the multiplicity finding.** It does not: it is a plan item and its recorded answer, in a checkpointed plan doc. Rewriting a landed checklist to match its own findings would erase the record of what was checked. Dismissed.
- **cubic (P3) — static coverage asymmetry across the three bundles.** Observation true, stated consequence false: static 34 is covered by `buildFixtureGraphs` and `parity`, both of which auto-discover the directory, and parity is a stronger structural check than the layout fingerprint. `STATIC_SNAPSHOT_KEYS` is a documented selection, and `34` is a flat parallel — the same shape as `STATIC_WIDE_PARALLEL`, already listed. Dismissed **but** it exposed something real: `STATIC_LIVE_COUNTERPARTS` mirrors the `StaticVsLive` stories one-for-one and never said so, which is what made it misreadable. Now stated in a comment — the one code change from this round.

### Pre-landing review (gstack `/review`)

Verdict: ship-ready, nothing blocking. Five informational findings; four fixed, one deferred. The two that mattered:

- **The assertion labelled "the upstream-addition detector" never read the corpus.** It iterated `CORPUS_NATIVE_CODES` — the hand-written in-repo list — so it could only fail after a human edited that list, which is exactly the dependency the oracle exists to remove. Detection did still happen, via the set-equality assertion, but as an array diff with none of the wording the failure guide promises. **This also explains why my own negative test passed:** deleting a code from the catalog exercised a proxy, not the real scenario. Now iterates the codes found in the corpus, and re-tested properly by injecting an unknown `native.Duration` into a fixture registry — fails with "Duration is a code the catalog knows".
- **The docs overclaimed what the oracle covers.** A dry spec's `concept_registry` holds the concepts that spec _references_, not every native pipelex knows (`pipeline_01` contributes only `Text`). The plan scoped this correctly ("for every code the corpus reaches") and so did the test file, but `docs/static-graph.md` and `CHANGELOG.md` dropped the qualifier — and the docs section then contradicted itself four lines later. Both now claim only what the mechanism delivers.

Also fixed: `Makefile`, `CLAUDE.md`, and the generator docblock still advertised a full-corpus `make fixtures-live` and `fixtures-live-missing` as working (the latter now selects exactly `pipeline_32`/`33` and always fails); `--from-disk` was undocumented; the `omitted` hoist left over from the barrel rework moved back inside its block; new doc prose unwrapped per the no-hard-wrap rule; a hardcoded test count removed from this doc — the very rule that let it go stale.
