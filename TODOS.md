...
**It cannot be pinned by a story, and finding that out was worth the detour.** Two attempts failed before the reason became clear: the gate is only observable through a file input that does not gate readiness, because a required singular file leaves readiness unmet for the whole upload and Run is blocked regardless. A plural renders a list control with no `<input>` until an item is added; pre-filling a required file replaces the dropzone with the filled state. A sweep of every generated contract then gave the real answer — **the corpus contains no optional or plural file input at all**. Writing one would mean hand-authoring a contract, which the generated-fixtures rule forbids. So the fix rests on the documented contract, and the gap is recorded rather than papered over.
