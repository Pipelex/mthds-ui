...
**Deviation from plan:** the full `validateGraphSpec` is wired into the standalone load path only, not into `buildGraph`/`GraphViewer` — wiring it there would require regenerating every Storybook pipeline fixture (`_generated.ts`) from pipelex 0.28.0 `.mthds` sources. Library consumers should call the exported `validateGraphSpec` when they load a spec. To keep `buildGraph` safe regardless, the three internal pipe-call narrowing points (`graphBuilders`, `graphControllers`, `graphFolds`) use `asPipeCallNode()` — a checked narrowing exported from `validateGraphSpec.ts` that throws a greppable `GraphSpecValidationError` on a malformed node rather than a bare `TypeError`.
