...
- **Phase 9 (fixtures):** hand-written test/story fixtures migrated to the tightened types via a one-shot TS-API codemod (`kind`/`status`/`io`/edge `id` filled). _(Superseded: a follow-up effort replaced `_generated.ts` with generator-produced `_generated.dry.ts`/`_generated.live.ts` and wired the validator into `GraphViewer` — see the resolved deviation note below.)_
...
**Deviation from plan — RESOLVED.** The full `validateGraphSpec` is now wired into the React render path: `GraphViewer` validates its `graphspec` prop once via `useMemo` (mirroring `adapter.ts` for the standalone path), so every Storybook story runs through the validator. The blocker — stale fixtures — was cleared by a new `scripts/generate-fixtures.mjs` (`make fixtures` / `make fixtures-live`) that regenerates every `_generated.*.ts` spec from pipelex 0.28.0 `.mthds` sources. `buildGraph` itself stays internal-trusting; the three pipe-call narrowing points (`graphBuilders`, `graphControllers`, `graphFolds`) keep `asPipeCallNode()` as defense-in-depth.
