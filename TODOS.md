...
**One coverage gap the reshape exposed.** The corpus's only `fixed` multiplicity is on an OUTPUT (`pipeline_30`, `item_count: 5`). No fixture declares a fixed-count INPUT, so the kernel's new `Concept[N]` input behaviour — gating on the declared count, capping the row control, the `2 of 3 items` badge — is exercised by the kernel's tests and by nothing here. Same shortage as `deferred-kernel-fix-coverage.md` records: it needs a bundle the pipeline corpus does not have.
