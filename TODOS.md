...
**The version ranges are deliberately stale**, and this is the one thing a fresh session must not "fix". `@pipelex/mthds-form` is not published with this campaign — npm latest is `0.3.0` and the work lives on the kernel's `fix/Reported-issues` branch — so both ranges stay at `^0.3.0` and local work runs against `make use-local`. Moving them is a reviewed change owned by `/bump-mthds-form`, and it lands when the kernel publishes. Until then `npm install` gives a kernel too old for this source; that is expected, not a break to chase. The release ordering is `../wip/inbox/2026-08-24-workspace-form-kernel-release-cascade.md`: the hosted deploy gates the kernel release, which gates all three consumer bumps.
