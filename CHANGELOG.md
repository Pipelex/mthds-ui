...
- **The React entries no longer load the form kernel's stylesheet (Breaking)**: `./graph/react` and `./form/react` stopped importing the kernel's prebuilt sheet, because no single position in the cascade served every host (`docs/run-form-panel.md` explains why). A Tailwind 4 host now follows the kernel's documented Tailwind 4 setup with `@import "@pipelex/mthds-ui/tailwind.css"` in place of its `@source` line, and no longer declares `@pipelex/mthds-form` or names the `mthds-form` layer; a host without Tailwind imports `@pipelex/mthds-ui/form-kernel.css` once, or the controls in `RunPanel` and in the graph's detail panel render unstyled. Tailwind 3 hosts and Tailwind 4 hosts with a prefix are not supported.

- **Bumped `@pipelex/mthds-form` to `0.11.0` (was `0.8.0`) (Breaking)**: the kernel is a dependency of this package, so the range moving from `^0.8.0` to `^0.11.0` changes the build every host resolves, and a caret range below 1.0 does not bridge a minor. What a host notices: the prebuilt sheet now reads every token with a fallback holding the light palette, so a host without Tailwind that imports `@pipelex/mthds-ui/form-kernel.css` and defines no tokens gets the kernel's light neutral controls, where under `0.8.0` its surfaces and switches rendered transparent; in `RunPanel`, a file input on a panel given no `uploadFile` offers a link field instead of a dropzone that accepted a file and silently discarded it; in the graph's detail panel, a result table shows five columns and moves the rest into the row's detail, an image inside a prose value renders as a link rather than loading on paint, a `data:` URL of a type outside the kernel's allow-list (`image/svg+xml` above all) is named rather than painted, and no default string shows an end user a `pipelex-storage://` reference. The kernel also brings `zod`, `@json-render/core`, `@json-render/react` and `@radix-ui/react-tabs` into a host's install, for its `./generative` entry, which nothing in this package imports; `@json-render/react` requires React `19.2.3`, so this package's `react` and `react-dom` peers rise from `^19` to `^19.2.3`, and a host on React 19.0 or 19.1 gets npm's `ERESOLVE` warning, or a failed install under `--strict-peer-deps`, until it upgrades. The documented Tailwind 4 setup now declares `@custom-variant dark (&:is(.dark *));`, as a shadcn/ui codebase already does, so a `dark:` utility in the kernel follows the `.dark` class rather than the operating system's preference.
