...
- **Storybook takes the form kernel's prebuilt styling lane** (`theme.css` + `styles.css`), making these stories the first place that lane is exercised end to end. Its Tailwind preflight is global, so the existing graph stories were measured with and without it. What moves is text metrics, and the cause is preflight's single `line-height: 1.5` on the root: card text goes from `normal` to a fixed line-height (`.pipe-card-header` to 24px at a 16px font) and a header box grows a few pixels. What does **not** move is the graph itself — `.react-flow__node` box heights are identical, because ELK sizes them. So Storybook remains a faithful surface for verifying layout, spacing and edge routing, and is not one for judging text fit or overflow inside a card, which no consumer of `@pipelex/mthds-ui/graph/react` sees this way.
...
- **`RunPanel`'s `env` prop documents what it actually does.** Both the prop's own contract and the props table in `docs/run-form-panel.md` said the host's value wins per key and the panel fills in only what was left undefined — which stopped being true for `uploadingIds` when that key became a union of the host's set and the panel's, precisely so an upload the panel started cannot be hidden by a tracker the host also passes. The per-key rule still holds for `disabled`, `onDropFile` and `resolveUrl`; the exception is now stated where a consumer reads it rather than only in a comment beside the implementation.

- **`RunPanel`'s `running` prop now states when it has to be set, not just what it means.** It is the only thing standing between a second submit and a duplicate execution, and `blocked` is computed during a render — so a host that flips `running` after its API answers rather than synchronously inside `onRun` leaves Run enabled for the whole round trip, and a second click there passes the same check the first one did. The panel cannot close that itself: it is told when a run starts and never that one finished, so an internal lock would have no release and a host that never passes `running` would be wedged after its first run. `docs/run-form-panel.md` promised "you still cannot start a duplicate run" without saying what that rests on; it now says, and the prop's own doc asks for the value synchronously.
...
- **A host's `translate` now renders the whole error summary, not the part of it the kernel happened to produce.** The summary has three routes — the variables the verdict named, the ajv errors it described, and a last-resort line when there is neither — and only the middle one went through the translator. The other two were hardcoded English, so a localized host got a French form that answered a blocked run in English on some inputs and in French on others, with nothing about the form to predict which. The panel cannot choose the route: the kernel's verdict does, and it has already reclassified the same input from one route to another across kernel versions — so this was never a branch that could be left in English safely, only one that had not been reached yet. `translate` is now keyed on `RunPanelMessageKey`, which nests the kernel's own `ValidationMessageKey` inside the two keys the panel owns (`runPanel.missingInputs`, which receives the variable names as an `{inputs}` value so a host's grammar can place them, and `runPanel.fillRequired`). Nesting rather than duplicating the kernel's union is what makes a key the kernel ADDS fail the string table's exhaustiveness check instead of silently rendering `undefined` at a user.

  Worth recording why the gap survived having a test written specifically for it. At the kernel version this panel pins, readiness and the gate agree about which inputs are missing, so every form that would produce a named variable has Run disabled and never reaches the summary — the untranslated branch was unreachable from the UI, and the story covering the seam could only ever exercise the branch that worked. The route is therefore pinned in `runGate.test.ts`, where it can actually be exercised, rather than in a story that would have to wait for a kernel change to become honest.

- **An optional input no longer disappears from under the cursor when you clear it.** Which optionals the collapsed fold shows was derived from the live value, so emptying a filled one flipped it to "foldable", dropped it out of the rendered set, and unmounted the very input being typed into — focus lost, and every keystroke after it going nowhere. It needed no host pre-fill to reach: expand the fold, fill an optional, collapse it (it stays on screen because it is filled), then clear it. The revealed set is now latched, so while the fold is collapsed the form's shape only ever grows; it is recomputed when the fold reopens or the contract changes, which are the two moments nobody is mid-edit. The latch is state adjusted during render rather than a ref written during render, which matters in a library component: a ref write survives a render that never commits, so a host rendering the panel inside a transition — App Router navigation does that by default — could have an abandoned render for one contract wipe the set the committed contract is still displaying from, re-entering the same defect by another door. Keeping the field on screen is not permanent, which takes two things: the fold toggle counts what a collapse _would_ hide (every empty optional) rather than what is hidden right now, so emptying one brings the control back; and the toggle's own handler clears the latch, so using it actually folds the field away. Without the pair, an optional you emptied stayed in the form for the rest of the contract's life with no control that would remove it. Nothing about readiness or the wire changes — an emptied optional is omitted from the payload whether or not its input is still on screen.

- **`env.disabled` now holds the Run button, not just the fields.** A host setting it froze every input and left Run live and clickable beside them, so a click there started a run from a form the user could not edit. The panel already treats the host's flag and `running` as one concept where the fields are concerned — `fieldEnv.disabled` falls back from one to the other — so gating the button on only one of the two sources was an asymmetry, and the submit path inherited it because both read the same expression. It is read off the prop rather than off the resolved field value, because the two differ exactly when a host passes `disabled: false` while `running` is true, and there the button must still be held.

- **Two `requestSubmit()` calls in one synchronous block now start one run, not two.** The submit gate is computed during a render, so both calls closed over the same `blocked === false` and both reached `onRun` — including for a host doing exactly what the `running` prop asks and setting it synchronously inside `onRun`, because React has not re-rendered between the two calls. Two real clicks were never affected: they are separate tasks with a commit between them. What made this reachable rather than theoretical is that the programmatic path is a documented surface here, with a story asserting it respects every gate; the cost of the gap was a duplicate execution, which nothing downstream undoes. A submit that passes the gate now latches until the end of the current task. The release is a microtask, and that is what separates this from the lock the panel still refuses to take: a lock held for the LIFETIME of a run would have no release, since the panel is told when a run starts and never that one finished, so a host that never passes `running` would be wedged after its first run. A microtask is queued unconditionally and always runs, so the latch holds nothing across a render and cannot wedge — pinned by a story whose host schedules no state update at all and still runs on every click. Setting `running` synchronously remains the host's job for every window longer than one task, and `docs/run-form-panel.md` now separates the two windows instead of declining both.

- **The Run button's label now clears WCAG AA in both themes.** White on the palette's `--color-accent-strong` missed the 4.5:1 a 13px/600 label requires, in the light theme the panel defaults to and in the dark one alike. The background moves per theme and the label stays white, which is the half of the pairing worth changing: reaching instead for `--text-on-accent` — the token that looks like the intended fix — repairs only the dark theme, because that token is calibrated against a different colour and resolves to white in the light palette. The colours are literals, and that is the trade the change accepts: a host overriding `--color-accent-strong` no longer re-themes this button along with the graph. It is the primary call to action and the only place in the library that puts text on that token, so nothing else shifts as a consequence, and the palette itself is left alone rather than darkened to fix one button. `src/form/__tests__/contrast.test.ts` reads the literals back out of the stylesheet and holds them to the AA line, including refusing a `var()` in their place — the failure is otherwise silent, since the button renders perfectly well when nobody can read it.

- **The packaging smoke test can now see the regression it exists for.** It checked that every exported stylesheet RESOLVES — which the build's own copy step guarantees whether or not the JS still imports it — so removing `RunPanel.css` from tsup's `external` array, the exact shape of the v0.4.0 `GraphToolbar.css` release, left the script reporting every check `ok` while the panel shipped with class names and no styles. It now walks the installed module graph and asserts each exported stylesheet is actually imported by the JS that needs it, and reports a relative import that resolves to nothing instead of walking silently past it (which made every downstream check pass vacuously for that entry). Verified by rebuilding with the regex removed: the existence check still passes and the new one fails. The `"use client"` assertion also covers **every** React entry now rather than one of the two — and the one it used to skip is the one standing on the bundler alone, since only the other gets the `onSuccess` re-prepend.

- **A dynamic `import()` of the form kernel is now caught by both isolation guards.** Base `no-restricted-imports` does not visit `ImportExpression` at all, and the smoke test's scanner could not match across the parenthesis, so `await import("@pipelex/mthds-form")` in a graph module would lint clean, stay external through the build, pass the packaging check, and then fail at runtime for every graph-only consumer — who by construction has not installed the kernel. Lazy-loading an optional peer is the obvious thing to reach for, which is what made the shared blind spot worth closing rather than theoretical.

- **The standalone CSS manifest guard works on Windows and in both directions.** It compared `path.relative` output against forward-slash constants, so on Windows the exclusion silently stopped excluding and the suite failed while recommending the one change that would actually be wrong. And the exclusion was only ever enforced one way: forgetting to list a stylesheet failed loudly, while listing one the standalone bundle cannot render passed just as quietly — the mistake with the real cost, since it inlines dead CSS into every standalone HTML. Both directions are asserted now.

- **A decimal in a number field no longer vetoes the run silently.** The panel's `<form>` had no `noValidate`, and the kernel's number control carries `step={integer ? 1 : 0.1}` — so any float with more than one decimal is a native `stepMismatch` even though `native.Number` accepts it and the kernel's own validation pass does too. Interactive validation then aborted submission before `onSubmit` ran at all: no gate, no `onRun`, no error from the panel, and a native message that is wrong about the domain ("nearest valid values are 87.2 and 87.3"). It was reachable from a contract the stories already render — `recruitment.MatchScore.score` is a plain `number` — and it contradicted this repo's own documented promise that a host may drive the form with `requestSubmit()`. The form now sets `noValidate`, which costs nothing: every constraint that should hold is checked by the run gate. Worth recording how it hid: `userEvent.click` does **not** run interactive validation, so the whole existing story suite reported successful submits against the broken form; the regression story uses `requestSubmit()`, which does, and which is the path hosts are told to use.
- **Run is held by the panel's own uploads even when the host reports its own.** `env.onDropFile` and `env.uploadingIds` default independently per key, so a host can hand the panel `uploadFile` — letting it own the drop, and mark the field in its own set — while also passing an upload tracker of its own. The host's set replaced the panel's rather than joining it, so a tracker that happened to be idle for this form hid an upload the panel itself had started, and Run stayed live through it. `docs/run-form-panel.md` already described the union — "any upload the panel knows about, **including** uploads you report through `env.uploadingIds`" — so the code contradicted its own documented contract. The sets are now merged. A host that owns the whole upload loop supplies `onDropFile` too, which leaves the panel's set empty, so the union is exactly that host's set and "yours win" still holds where that clause applies. Pinned by a story over `cv_matching.screen_cvs`, whose `cvs` is a LIST of documents — the only shape in which this gate is observable at all, since a required singular file leaves readiness unmet for the whole upload and Run is disabled whether the gate works or not.
- **A partial fixture refresh no longer re-sources the vendored corpus entries.** The generator has two ways to select a subset — `--only` and `--missing` — and both limited the pipeline loop while leaving the fixture assembly that follows unrestricted, so `make fixtures-contracts ONLY=pipeline_05` and `make fixtures-missing` alike re-ran both corpus entries through whatever pipelex the local venv happens to be: rewriting fixtures outside the requested selection and mixing fixture versions, which is the hazard the LIVE path is already documented against. They are now skipped on any partial run, which is the rule the pipelines already followed — anything unselected is read from what is committed beside it rather than re-run — and the run prints which entries it reused rather than re-sourced, so the narrowed scope is stated instead of silent. "Partial" is now one named predicate declared once, rather than the two flags spelled out longhand at each site, which is how the first fix came to name one and forget the other. The contracts pass refuses `--missing` outright, since `--only` is the only subset it knows how to take.
- **Each fixture-generator mode now demands exactly the executable it invokes.** The generator needs two, satisfied separately: graph specs and inputs templates go through the pipelex CLI (`PIPELEX_BIN`), while the `pipe_io_contracts` dump has no CLI surface and shells out to the venv interpreter (`PIPELEX_PYTHON`). Both default to the same sibling venv, and that coincidence hid a guard that checked the wrong thing in both directions at once: `--contracts` asserted the CLI it never calls, so it refused a machine with a working interpreter and no sibling checkout — precisely the case `PIPELEX_PYTHON` exists to serve, and precisely the offline-and-fast property that makes the contracts pass its own mode — while the mirror image, a CLI with no interpreter, sailed past the guard and died inside `execFileSync` with a bare `ENOENT` naming neither the variable to set nor the mode that needed it. Guards now sit on the executable each path actually runs, and up front rather than partway through, after the run artifacts have already been rewritten. `--check` asserts only the CLI, since it writes nothing and so never dumps contracts; `--from-disk` asserts neither and completes with both absent. The two-executable contract is written down in `CLAUDE.md` rather than left to be discovered from an error message. Both are also located correctly on native Windows now: the script already branched on `win32` to add `.exe`, but kept looking inside `.venv/bin`, which describes no virtualenv that has ever existed — a venv puts its executables in `Scripts` there. A branch that changes only the filename is worse than none, because it looks handled while resolving to a path that cannot exist, so the directory and the suffix now move together for the CLI and the interpreter alike.
