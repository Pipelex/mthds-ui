...
- **A targeted contracts refresh no longer re-sources the vendored corpus entries.** `--only` limited the pipeline loop but not the fixture assembly that follows, so `make fixtures-contracts ONLY=pipeline_05` also re-ran both corpus entries through whatever pipelex the local venv happens to be — rewriting fixtures outside the requested selection and mixing fixture versions, which is the hazard the LIVE path is already documented against. They are now skipped whenever `--only` is set, which is the rule the pipelines already followed: anything unselected is read from what is committed beside it rather than re-run. The run prints which entries it reused rather than re-sourced, so the narrowed scope is stated instead of silent.
