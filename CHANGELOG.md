...
- **Bumped `@pipelex/mthds-form` to `0.11.0` (was `0.8.0`) (Breaking)**: the kernel is a dependency of this package, so the range moving from `^0.8.0` to `^0.11.0` changes the build every host resolves, and a caret range below 1.0 does not bridge a minor. What a host notices: the prebuilt sheet now reads every token with a fallback holding the light palette, so a host without Tailwind that imports `@pipelex/mthds-ui/form-kernel.css` and defines no tokens gets the kernel's light neutral controls, where under `0.8.0` its surfaces and switches rendered transparent; in `RunPanel`, a file input on a panel given no `uploadFile` offers a link field instead of a dropzone that accepted a file and silently discarded it; in the graph's detail panel, a result table shows five columns and moves the rest into the row's detail, an image inside a prose value renders as a link rather than loading on paint, a `data:` URL of a type outside the kernel's allow-list (`image/svg+xml` above all) is named rather than painted, and no default string shows an end user a `pipelex-storage://` reference. The kernel also brings `zod`, `@json-render/core`, `@json-render/react` and `@radix-ui/react-tabs` into a host's install, for its `./generative` entry, which nothing in this package imports. The documented Tailwind 4 setup now declares `@custom-variant dark (&:is(.dark *));`, as a shadcn/ui codebase already does, so a `dark:` utility in the kernel follows the `.dark` class rather than the operating system's preference.
