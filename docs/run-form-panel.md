...
**Run is disabled while any upload the panel knows about is in flight**, including uploads you report through `env.uploadingIds`. Readiness alone is not enough, because a non-gating file input — an optional one, or a plural one, since a list never gates — never counts toward readiness at all: without this, Run stays live right through such a field's upload and the method runs with the file simply missing. The gate lives on the submit path as well as on the button, so `form.requestSubmit()` — which ignores the disabled button entirely — is held too: run the form from your own button if you like, and you still cannot send it without a file that is on its way.
