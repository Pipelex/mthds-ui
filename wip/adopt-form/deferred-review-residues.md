...
**The fixed case IS pinned by a story, and the earlier claim here that it could not be was simply wrong.** What this note said before — that a sweep of every generated contract found no optional _or plural_ file input anywhere in the corpus — is false on the plural half. `cv_matching.screen_cvs` takes `cvs`, a list of `native.Document`, and `cv_batch_screening.batch_analyze_cvs_for_job_offer` takes the same shape. The claim came from probing `document_analysis.analyze_pages.pages`, which is plural but is `native.Page` — a composite concept, so it renders an object control and no dropzone. One wrong probe, generalised into a property of the whole corpus.

The gate is only observable through a file input that does NOT gate readiness, and that part was right: a required singular file leaves readiness unmet for the whole upload, so Run is blocked either way and the bug is invisible. A LIST satisfies it, since `mustBeFilled` excludes lists. `HostTrackerJoinsPanelUploads` renders `screen_cvs` with `job_offer` already filled and an idle `env.uploadingIds`, adds a row, drops a file into it, and asserts Run goes disabled. Verified red-then-green: reverting the merge to `env?.uploadingIds ?? uploadingIds` fails exactly that assertion and nothing else.

**The `onDropFile`-alone case stays unpinned and unchanged**, and now for a reason rather than for lack of a contract to render: the assertion such a story would make is that Run stays live, which is the documented consequence of a host supplying half the pair, not a defect worth locking in. `docs/run-form-panel.md` already states the requirement — pass `env.onDropFile` _and_ `env.uploadingIds` — so this is an enforcement gap, not a contradiction. Closing it means a `process.env.NODE_ENV !== "production"` warning when exactly one of the pair is supplied, and a dev-only console warning is a new kind of thing for this library: there is no other one, and `no-console` is an eslint error here, so it would need an explicit exemption. Worth doing if a host trips over it; not worth inventing the category speculatively.
