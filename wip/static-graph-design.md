...
- **Condition outcome dedupe.** One child node per distinct _target pipe_, not per outcome value, mirroring the runtime tracer; outcomes routing to the same pipe (typically one value plus `default_outcome`) merge into a single child whose `tags.outcome` / `contains` label carries all routing values joined with `" | "`.
...
**`combined_output` — resolved, with an upstream surprise.** The known question (the dry run registers the _controller_ as the combined stuff's producer, but mthds-ui only takes producers from non-controller nodes) is now pinned by an explicit test: on BOTH paths the combined stuff is producer-less in the analysis, consumed downstream by digest, and fed by `parallel_combine` stuff-to-stuff edges. Phase 2 now classifies `parallel_combine` targets as combined stuff in the renderer instead of external input stuff, applying equally to dry and static graphs. No fixture bundle could be added to exercise it end-to-end because **pipelex deleted `combined_output` upstream** (pipelex #1014, "PipeParallel always combines: required main stuff invariant", adds `native.Composite`) _after_ the fixture corpus was generated: pipelex 0.36.0 rejects the field on authored bundles while the MTHDS spec (`mthds/docs/spec/`) still documents it. Consequences, tracked as follow-up:
...
- `data/schema/mthds_schema.json` (and the mthds spec) still carry `combined_output`; the static builder keeps supporting it — the MTHDS spec is its normative source, and spec-vs-pipelex divergence is an upstream question to settle in the `mthds/` repo, not here.
  - **Superseded as of PR #63.** Refreshing the bundled schema to pipelex v0.41.0 dropped `combined_output` from `PipeParallelBlueprint`, and with `additionalProperties: false` that copy now _rejects_ the key — so the premise above ("the schema still carries it") no longer holds. The registry type was made optional; whether the static builder should keep honoring the authoring key is now an open decision, written up in [`pr-63-review-notes.md`](./pr-63-review-notes.md).
...
Phase 2 landed the renderer contract around explicit graph modes:

1. **Static display mode.** `GraphSpec.meta` has `mode?: "dry" | "live" | "static"`, and static behavior keys only on `meta.mode === "static"`. Missing mode remains a legacy runtime graph. Static pipe cards suppress status dots, status titles, and pulse animation. Static detail panels hide status, duration, metrics, and execution-data dumps, while keeping authored blueprint sections, IO, concept links, descriptions, errors, and tags. `statusMap` overlays are ignored for static cards.
2. **`PipeSignature` audit and story.** `PipeSignature` keeps the existing muted/dashed card treatment and the detail copy "Signature stub — declared but not yet implemented." A focused static signature story now covers it without relying on a dry run.
3. **Condition outcome labels.** The primary presentation is a child-card badge from `node.tags.outcome`, not an edge label. The builder still labels `contains` edges for data fidelity, but the visual policy is card-attached so it survives routing changes and folded controller cards.
4. **Batch multiplicity badge.** Static `PipeBatch` nodes and their representative branch nodes carry `tags.batch_multiplicity` and render `xN`, `xmany`, or `x?` (unknown fallback) from the declared list input multiplicity. Expanded graphs show the badge on the representative branch; folded batch cards show it on the card.
5. **Combined-output role classification.** `parallel_combine` target stuffs are classified as `combined`, not `input`, so producer-less combined outputs no longer look like external inputs. This applies to dry and static graphs.
6. **Static fixture catalog.** `STATIC_*` specs and `STATIC_RUN_CATALOG` live next to the dry/live catalogs and are built from raw checked-in `.mthds` bundles through `buildStaticGraphSpecFromToml` — no CLI, gateway key, Python, or network. Selected static structural snapshots are deterministic.
...
**Phase 2 — UI adaptations.** Complete in this repo. `meta.mode` handling, static display chrome, condition-outcome labels, batch badge, combined-output role classification, `PipeSignature` audit/story, `STATIC_*` fixture catalog, snapshots, and docs all landed. The builder remains in `src/static-graph/`; there is no planned extraction to `mthds-js`.
