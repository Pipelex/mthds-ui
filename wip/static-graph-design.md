...
4. **Recurse per controller type:**
   ...
   - **PipeParallel** — recurse into every branch with the same inherited scope (branches are independent). `add_each_output` → each branch's output binds under its `result` in the enclosing sequence's scope. `combined_output` → mint a combined stuff (concept = the parallel's declared output) and emit `parallel_combine` edges from each branch's output stuff to it; the combined stuff is the controller's output. Without `combined_output`, the controller's `io.outputs` carries **all** branch outputs (same digests — matches the dry tracer), and the primary output (what a `result` binds to) is the last producing branch's stuff.
   - **PipeCondition** — recurse into every outcome pipe and `default_outcome` (skip `fail`/`continue` — outcome actions, not pipe refs), all with the inherited scope. `add_alias_from_expression_to` → bind an alias stuff typed `native.Dynamic`. The outcome value is recorded per child (`tags.outcome` on the child node + `label` on its `contains` edge — the tracer never surfaces this; static can). The controller's representative output = the default branch's stuff when present, else the first producing outcome's.
