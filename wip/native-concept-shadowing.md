...
> **Fixed (2026-08-11)** in [PR #69](https://github.com/Pipelex/mthds-ui/pull/69). The filter quoted below is gone: `SchemaTable` renders every field in every mode, and dry-run payload suppression (`hasData`) is untouched. The question this section raised — does a required-only filter belong on the Structure tab — was answered no, and it was answered on its own rather than with §4, since §4 is about enriching a cell and this was about a row withheld outright. §4 is still open. Everything below is the record as written at the time.
