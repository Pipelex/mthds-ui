...
> `HostTranslatesTheErrorSummary` went red asserting the host's `translate` reached the error summary. The cause was not the kernel. `summarizeVerdict` in `src/form/runGate.ts` has two branches, and **only the fallback one passes `t` through** — the `missingInputs` branch returns a hardcoded English `` `Missing required fields in: …` ``. Under `0.2.0` that story's malformed input produced an empty `missingInputs`, so it exercised the translated fallback; under `0.3.0` the validator names the input (`["invoice"]`), the first branch runs, and the untranslated line surfaces. The story existed _specifically_ to cover the i18n seam and had never once exercised the branch that breaks it.
