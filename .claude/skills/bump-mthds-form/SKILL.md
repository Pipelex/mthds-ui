...
1. **The kernel is an ordinary `dependency`, declared once.** It was an optional peer named twice (`peerDependencies` + `devDependencies`) until `4462773` collapsed it to a single registry range under `dependencies`. A host therefore installs it transitively and never names it — which is also why this library's copy of the kernel is the one whose styling reaches the page: a Tailwind 4 host compiles its classes through `@pipelex/mthds-ui/tailwind.css`, which scans that copy, and a host without Tailwind loads its prebuilt sheet through `@pipelex/mthds-ui/form-kernel.css`.
2. **Moving that range is a breaking change for consumers of `@pipelex/mthds-ui`.** It changes which build of the kernel a host resolves, and with it the classes a host compiles or the prebuilt sheet it loads through this library. That earns a changelog bullet in this repo's own terms — the starter app never faces this, because it has no consumers.
...
| **The Storybook styling lane**       | `.storybook/preview.ts` imports `theme.css` + `../src/styles/form-kernel.css` (the cascade-layer wrapper, never `styles.css` raw); `.storybook/main.ts` names `@pipelex/mthds-form` and `@pipelex/mthds-form/react` in `optimizeDeps.include` | Storybook here is a host without Tailwind and takes that host's lane (`docs/run-form-panel.md`, "A host without Tailwind"). A moved or renamed CSS entry point makes the stories render unstyled and _still pass_. A new entry point imported by a story and missing from `optimizeDeps.include` fails story tests with "Failed to fetch dynamically imported module" — the reason that list exists is written in the file.                                                                                                                                                 |
| **The Tailwind 4 host lane**         | `src/styles/tailwind.css` (the two `@source` paths into the kernel's `dist`), and the minimal Tailwind 4 host in `scripts/smoke-pack.mjs`, whose oracle is the kernel's own prebuilt `styles.css` | A kernel release that moves its `dist`, adds a token, uses a `dark:` variant or depends on another animation package shows up as selectors the minimal host fails to compile. Investigate each one: a class the kernel compiled only from a word in its source comments goes in `SOURCE_ONLY_KERNEL_SELECTORS` with its reason, and anything else means the kernel's documented Tailwind 4 setup changed, so the README's "Styling the form controls" and the smoke host's stylesheet change with it. Never loosen the comparison. |                                     |
