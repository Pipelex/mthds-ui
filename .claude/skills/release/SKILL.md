...
- **Also stamped:** nothing. The number lives in `package.json` and the lock alone — no version literal in `src/`, none in the README, none in `docs/`. The repo's toolchain is npm, and `package-lock.json` is the only lockfile (the smoke gate below also installs a throwaway consumer with pnpm, outside the repo, and writes no lockfile here): a stale `pnpm-lock.yaml` was tracked until it was deleted for carrying no entry for the form kernel and still listing a dependency dropped in v0.20.0.
...
3. **`make smoke-pack`** — `node scripts/smoke-pack.mjs`, which packs the tarball, installs it into throwaway consumers, and inspects it from the outside: that every declared export resolves to a file that ships, that every stylesheet the JS needs is imported by it while the two a host imports itself, `tailwind.css` and `form-kernel.css`, are imported by none, that both React entries keep their `"use client"`, that the form kernel arrives as a dependency in exactly one copy, that the React-free entries never reach it, and that a minimal Tailwind 4 host importing `tailwind.css` compiles every utility in the kernel's prebuilt sheet under npm with the kernel hoisted, npm with it nested, and pnpm. **It needs `pnpm` on the path** and stops before packing when it is missing; `corepack enable` puts it there on Node 22 and 24, which bundle corepack, and a later Node needs corepack installed first. **Nothing in CI runs it** — not on the pull request, not on the push — and the tarball is the entire product of this release, so it is the release's own gate. It is slow (it builds through `npm pack`'s `prepare` hook and installs from the registry, so it needs the network) and it exits non-zero listing the checks that failed. Red blocks the release: every property it covers has a history of failing silently, which is why it exists.
